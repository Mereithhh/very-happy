...
- 最新 tag / npm `very-happy-cli` = **v0.2.98**（mac-office daemon 同版本；2026-09-02；含 B-275 认证失败换进程 + B-276 Claude 登录预检/诊断/`credentialStore=file`）；线上 Web = `main@cdcb7ccf`（2026-09-02 蓝绿）。`packages/happy-cli/package.json` 里的 version 不是发布版本（发版脚本按 tag 定），别拿它判断线上版本。
