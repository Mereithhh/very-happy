...
- 最新 tag / npm `very-happy-cli` = **v0.2.96**（mac-office daemon 同版本；2026-09-01；含 B-272 单写者锁，0.2.95 首发）；线上 Web = `main@3ad12ac7`（2026-09-02 蓝绿，deploy run 33537526006，含 B-272 changelog）。`packages/happy-cli/package.json` 里的 version 不是发布版本（发版脚本按 tag 定），别拿它判断线上版本。
