...
export const en = {
    ...
    teams: {
        ...
        cancelled: "Cancelled",
        unavailable: "Teams are unavailable on this server, or this team no longer exists. Update the server or return to the team list.",
        sessions: "Sessions",
        teams: "Teams",
        refresh: "Refresh",
        retry: "Retry original action",
        intro: "Coordinate coding agents around one goal. Follow progress and take over here.",
        name: "Team name",
        machine: "Machine",
        selectMachine: "Select a machine",
        create: "Create team",
        bots: "Teammates",
        joinHint: "Linking a session only registers its identity. The lead must join through the Teams CLI and skill to delegate work.",
        lead: "Lead",
        ...
        delegate: "Delegate",
        tasks: "Tasks",
        result: "View submitted result",
        accept: "Accept result",
        body: "Instructions or reason",
        action: "Action",
        message: "Send message",
        return: "Return for changes",
        cancel: "Cancel task and descendants",
        execute: "Apply",
        delivered: "Delivered",
        waitingDelivery: "Pending delivery",
        unknown: "Outcome unknown. Inspect the session before starting another worker.",
        assignee: "Assignee",
        cleanup: "Cleanup",
        failed: ({ reason }: { reason: string }) => `Action did not complete: ${reason}. Please retry.`,
        counts: ({ bots, tasks }: { bots: number; tasks: number }) => `${bots} teammates · ${tasks} tasks`,
    },
    tabs: {
    ...
    session: {
        ...
        chat: {
            ...
            queueIntervene: 'Steer',
            queueDeliveryFailed: 'Message not queued. Your draft is kept; retry when ready.',
            queueMore: 'Queued message options',
    ...
    workspaceGuide: {
        ...
        groups: {
            ...
            organizeSummary: 'Scratch notes, your Todo panel and scheduled or triggered automations',
        ...
        capabilities: {
            ...
            automationsDescription: 'Open Automations from the sidebar to schedule or trigger agent sessions and scripts on one machine (cron with time zone, interval, once, or fire-by-event). Runs that fail, expire or wait for you appear under “Needs my decision” on the task board.',
    ...
    tools: {
        ...
        // B-499: very-happy's own tools (teams, automations, sessions, clipboard…)
        // render as a verb + human detail instead of raw JSON.
        builtin: {
            labels: {
                change_title: 'Title changed',
                copy_to_clipboard: 'Copied to clipboard',
                open_preview: 'Open preview',
                report_progress: 'Progress',
                team_create: 'Create team',
                team_join: 'Join team',
                team_inspect: 'Inspect team',
                team_delegate: 'Delegate task',
                team_message: 'Team message',
                team_submit: 'Submit result',
                team_accept: 'Accept task',
                team_return: 'Return task',
                team_cancel: 'Cancel task',
                team_handoff: 'Hand off task',
                team_schedule_create: 'Create reminder',
                team_schedule_pause: 'Pause reminder',
                team_schedule_resume: 'Resume reminder',
                team_schedule_cancel: 'Cancel reminder',
                automation_list: 'List automations',
                automation_get: 'Read automation',
                ...
                automation_fire: 'Fire automation',
                automation_runs: 'List runs',
                automation_report: 'Report run',
                automation_ack: 'Acknowledge run',
                sessions_list: 'List sessions',
                session_read: 'Read session',
                session_send: 'Send to session',
                ...
                journal_append: 'Journal note',
            },
            fields: {
                title: 'Title', text: 'Text', path: 'Path', mode: 'Mode', progress: 'Progress', attention: 'Attention',
                name: 'Name', newName: 'New name', id: 'Id', machine: 'Machine', team: 'Team', bot: 'Bot', goal: 'Goal',
                acceptance: 'Acceptance', assignee: 'Assignee', agent: 'Agent', model: 'Model', directory: 'Directory',
                parentTask: 'Parent task', task: 'Task', recipient: 'Recipient', body: 'Message', result: 'Result',
                reason: 'Reason', attempt: 'Attempt', schedule: 'Reminder', runAt: 'Runs at', interval: 'Repeats',
                version: 'Version', description: 'Description', trigger: 'Trigger', action: 'Action', prompt: 'Prompt',
                command: 'Command', sticky: 'Sticky key', concurrency: 'Concurrency', maxRuntime: 'Max runtime',
            ...
            trigger: {
                manual: 'manual only',
                once: ({ at }: { at: string }) => `once at ${at}`,
                onceUnknown: 'once',
                interval: ({ every }: { every: string }) => `every ${every}`,
                intervalUnknown: 'on an interval',
                hourly: ({ minute }: { minute: string }) => `hourly at :${minute}`,
                daily: ({ time }: { time: string }) => `daily at ${time}`,
                weekdays: ({ time }: { time: string }) => `weekdays at ${time}`,
                weekly: ({ time, day }: { time: string; day: string }) => `weekly on day ${day} at ${time}`,
                monthly: ({ time, day }: { time: string; day: string }) => `monthly on the ${day} at ${time}`,
                cron: ({ expr }: { expr: string }) => `cron ${expr}`,
                cronUnknown: 'cron',
            },
            action: {
                spawn: ({ agent, dir }: { agent: string; dir: string }) => `start a ${agent} session in ${dir}`,
                send: ({ session }: { session: string }) => `send into session ${session}`,
                script: ({ cmd }: { cmd: string }) => `run ${cmd}`,
            },
            status: { active: 'active', paused: 'paused' },
            runStatus: {
                queued: 'queued', claimed: 'claimed', running: 'running', done: 'done', failed: 'failed',
                skipped: 'skipped', expired: 'expired', cancelled: 'cancelled',
            },
            taskStatus: { queued: 'queued', running: 'running', submitted: 'submitted', done: 'done', cancelled: 'cancelled' },
            to: ({ target }: { target: string }) => `to ${target}`,
            chars: ({ count }: { count: string }) => `${count} chars`,
            lastN: ({ count }: { count: number }) => `last ${count}`,
            yes: 'yes',
            no: 'no',
            nextRun: ({ at }: { at: string }) => `next ${at}`,
            teamSummary: ({ name, members }: { name: string; members: number }) => `Team ${name} · ${members === 1 ? '1 member' : `${members} members`}`,
            inspectSummary: ({ name, tasks, bots, messages }: { name: string; tasks: number; bots: number; messages: number }) => `${name} · ${tasks} tasks · ${bots} members · ${messages} messages`,
            taskRef: ({ id }: { id: string }) => `Task ${id}`,
            scheduleRef: ({ id }: { id: string }) => `Reminder ${id}`,
            automationCount: ({ count }: { count: number }) => count === 1 ? '1 automation' : `${count} automations`,
            runCount: ({ count }: { count: number }) => count === 1 ? '1 run' : `${count} runs`,
    ...
    changelog: {
        ...
        releases: {
            sep25i: {
                ...
                rows: 'Each call is one line you can read at a glance: "Delegate task · to Codex · Ship the automations page", "Create automation · daily-inventory · Daily at 09:00 · Asia/Singapore", "Progress · needs review · …".',
                cards: 'Expanding a call shows its arguments as labelled fields (goal, acceptance criteria, trigger, action, prompt…), a digest of the result, and buttons that open the session, team or task it refers to; errors are shown plainly and the raw output stays one click away.',
                runners: 'Works the same whether the session runs Claude, Codex or pi; tools from other MCP servers keep the generic view.',
            },
            sep25h: {
            ...
            sep10g: {
                ...
                action: 'The pending rollout banner can request an update for one machine. Installation waits for idle and preserves sessions.',
                compatibility: 'Remote updates require CLI 0.2.133 or later on the machine; older versions show a pinned update command.',
            },
            sep10f: {
    ...
    automations: {
        // B-498: account-level automations (B-496) — list, detail and the
        // "needs my decision" band on the task board.
        title: 'Automations',
        ...
        statusActive: 'Active',
        statusPaused: 'Paused',
        concurrencySkip: 'skip when busy',
        concurrencyQueue: 'queue when busy',
        ...
        cancelRunConfirm: ({ name }: { name: string }) => `Stop the current run of "${name}"? The daemon abandons it on its next report.`,
        deleteAutomation: 'Delete automation',
        deleteConfirm: ({ name }: { name: string }) => `Delete "${name}" and all of its run history? A run in flight is abandoned.`,
        actions: 'Actions',
        // ---- toasts ----
        runQueued: ({ name }: { name: string }) => `Queued a run of ${name}`,
        acked: 'Acknowledged',
        ...
        emptyBody: 'Create one from any connected machine: very-happy auto create --name … --cron "0 9 * * *" --tz Asia/Singapore --spawn-dir ~/project --prompt "…"',
        notFound: 'This automation no longer exists',
        loadFailed: ({ code }: { code: string }) => `Could not refresh: ${code}. Showing the last known state.`,
        // ---- detail sections ----
        action: 'Action',
        actionSpawn: 'Spawn a session',
        actionSend: 'Send to a session',
        actionScript: 'Run a script',
        promptTruncated: 'Show full prompt',
        promptCollapse: 'Show less',
        stickies: 'Sticky sessions',
        noStickies: 'No sticky sessions — each run spawns a fresh session.',
        runs: 'Recent runs',
        noRuns: 'No runs yet',
        summary: 'Summary',
        error: 'Error',
        ...
        sourceManual: 'manual',
        // ---- attention reasons ----
        reasonNeedsInput: 'Agent is waiting for your input',
        reasonFailed: 'Run failed',
        reasonExpired: 'Run expired — the daemon stopped reporting or exceeded max runtime',
        reasonMachineOffline: 'Machine offline — nobody picked the run up',
        reasonUnknownOutcome: 'Daemon restarted before the outcome was known',
        reasonDaemonRestarted: 'Daemon restarted while the script was running',
        reasonInvalidAction: 'The daemon rejected the action (unknown agent or permission mode)',
        // ---- sidebar / grouping ----
        sidebarEntry: 'Automations',
        groupScheduled: 'Scheduled',
        groupTriggered: 'Triggers',
        groupScheduledHint: 'cron, interval and one-time runs',
        groupTriggeredHint: 'fired by an event: very-happy auto fire, MCP automation_fire',
        // ---- how to fire a trigger (detail page) ----
        fireTitle: 'How to trigger',
        fireBody: 'This automation has no schedule. Fire it from any machine or session — a payload becomes {{payload}} / {{payload.field}} in the prompt; the same dedupe key within 24h returns the original run.',
        fireCli: 'From a terminal',
        fireCliPayload: 'With a JSON payload and a dedupe key',
        fireMcp: 'From a managed session (MCP tool)',
        fireRunNowHint: '“Run now” above queues a run without a payload.',
        // ---- create / edit form ----
        newAutomation: 'New automation',
        editAutomation: 'Edit',
        formTitleNew: 'New automation',
        formTitleEdit: ({ name }: { name: string }) => `Edit ${name}`,
        create: 'Create',
        saveChanges: 'Save changes',
        created: ({ name }: { name: string }) => `Created ${name}`,
        saved: 'Saved',
        ...
        formLimits: 'Limits',
        fieldName: 'Name',
        fieldNameHint: 'lowercase letters, digits, - _ . — this is what you fire by',
        fieldDescription: 'Description',
        fieldMachine: 'Machine',
        fieldMachineHint: 'the daemon on this machine runs it',
        chooseMachine: 'Choose a machine',
        triggerCron: 'Schedule (cron)',
        triggerInterval: 'Every interval',
        triggerOnce: 'Once',
        triggerManual: 'Trigger only',
        triggerManualHint: 'No schedule. Fired by an event: very-happy auto fire <name> or the automation_fire MCP tool.',
        fieldCron: 'Cron expression',
        fieldCronHint: 'five fields: minute hour day month weekday, or @hourly / @daily / @weekly / @monthly',
        fieldTz: 'Time zone',
        fieldEvery: 'Every',
        fieldEveryHint: 'e.g. 15m, 1h30m, 1d (minimum 1m)',
        fieldAt: 'Run at',
        fieldAtHint: 'in this browser’s time zone',
        actionSpawnHint: 'Start a new session on the machine and send the prompt.',
        actionSendHint: 'Send the prompt into an existing session (it must be alive on this machine).',
        actionScriptHint: 'Run a command without a shell; stdout/stderr tail becomes the summary, exit 0 = done.',
        fieldAgent: 'Agent',
        fieldDirectory: 'Directory',
        fieldDirectoryHint: 'absolute path or ~/… on the machine',
        fieldModel: 'Model',
        fieldModelHint: 'optional — blank uses the agent default',
        fieldPermissionMode: 'Permission mode',
        fieldPermissionDefault: 'daemon default',
        fieldWorktree: 'Run in a fresh git worktree',
        fieldStickyKey: 'Sticky key',
        fieldStickyKeyHint: 'optional template, e.g. conv-{{payload.conversationId}} — runs with the same key continue the same session',
        fieldPrompt: 'Prompt',
        fieldPromptHint: 'templates: {{payload}}, {{payload.field}}, {{run.id}}, {{automation.name}}, {{now}}',
        fieldSession: 'Session id',
        fieldCommand: 'Command',
        fieldCommandHint: 'argv, shell-style quoting, no expansion — e.g. python3 sync.py --since "15 min"',
        fieldCwd: 'Working directory',
        fieldConcurrency: 'When the previous run is still going',
        concurrencySkipLabel: 'Skip this run',
        concurrencyQueueLabel: 'Queue it',
        fieldMaxRuntime: 'Max runtime',
        fieldMaxRuntimeHint: 'e.g. 30m, 6h — blank = default (6h, scripts 30m)',
        errName: 'Use lowercase letters, digits, - _ . (1–64 chars, starts with a letter or digit)',
        errMachine: 'Pick a machine',
        errCron: 'Five cron fields (or an @alias)',
        errTz: 'Unknown IANA time zone',
        errInterval: 'Between 1m and 366d, e.g. 15m or 1h30m',
        errOnce: 'Pick a date and time',
        errAgent: 'Pick an agent',
        errDirectory: 'Directory is required',
        errPrompt: 'Prompt is required',
        errSession: 'Session id is required',
        errScript: 'Command is required (check unbalanced quotes)',
        errMaxRuntime: 'Between 1m and 7d',
        errSchema: 'The server would refuse this shape — check the fields',
        errNameTaken: 'That name is already used by another automation',
        errInvalidCron: 'The server rejected this cron expression',
        errInvalidTimezone: 'The server rejected this time zone',
        errMachineNotFound: 'That machine is not on this account',
        errStale: 'Changed elsewhere — reload and edit again',
        cancel: 'Cancel',
        offlineMachine: 'offline',
    },
    ...
    cliUpdate: {
        ...
        requestAccepted: 'Update accepted. Installation will start when the machine is idle, without interrupting sessions.',
        requestFailed: 'The update request was not confirmed. Check the machine before retrying, or wait for automatic rollout.',
        legacyManualHelp: 'This machine’s CLI does not yet support remote manual updates. We recommend waiting for rollout; alternatively, run this command in its terminal.',
        ...
        pendingSummary: ({ machine, target }: { machine: string; current: string; target: string; count: number }) => `${target} is available for ${machine}, but automatic rollout has not started for this version. We recommend waiting for rollout, or you can update manually.`,
