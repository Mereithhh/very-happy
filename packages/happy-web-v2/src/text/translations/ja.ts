...
export const ja: PartialTranslationStructure = {
    board: {
        editTask: 'タスクを編集',
    },
    tabs: {
    ...
    settings: {
        ...
        about: 'このアプリについて',
        aboutFooter: 'Very HappyはCodexとClaude CodeのWebクライアントです。エンドツーエンド暗号化ではなく、信頼されたサーバーが中継内容にアクセスできる設計です。Anthropicとは提携していません。',
        whatsNew: '新機能',
    ...
    session: {
        ...
        completedChanges: ({ count }: { count: number }) => `${count} 件のファイルを変更`,
        viewDiff: '差分を表示',
        newChat: '新規チャット',
        setTitle: 'タイトルを設定',
        renameTitle: 'セッション名を変更',
        renamePlaceholder: 'セッションのタイトル',
    ...
    sessionInfo: {
        ...
        updateCliInstructions: 'npm install -g very-happy-cli@latest を実行してください',
    ...
    terminal: {
        ...
        publicKey: '公開鍵',
        encryption: '暗号化',
        endToEndEncrypted: 'サーバーを信頼する中継',
        acceptConnection: '接続を承認',
        connecting: '接続中...',
        reject: '拒否',
        security: 'セキュリティ',
        securityFooter: '信頼できるマシンで自分が要求したリンクのみ承認してください。中継サーバーは接続情報と中継データにアクセスできます。',
        securityFooterDevice: '信頼できるマシンで自分が要求した接続のみ承認してください。中継サーバーは接続情報と中継データにアクセスできます。',
    ...
    welcome: {
        ...
        subtitle: '信頼された中継サーバーは中継内容にアクセスできます。',
    ...
    machine: {
        ...
        offlineHelp: '• コンピューターがオンラインであることを確認してください\n• `very-happy daemon status`を実行して診断してください\n• 最新のCLIバージョンを使用していますか？`npm install -g very-happy-cli@latest`でアップグレードしてください',
        daemon: 'デーモン',
        status: 'ステータス',
        stopDaemon: 'デーモンを停止',
