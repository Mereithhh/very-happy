 ...
 */
export const pl: PartialTranslationStructure = {
    board: {
    ...
    liveStatus: {
        thinking: ({ elapsed }: { elapsed: string }) => `Myślę ${elapsed}`,
        waitingPermission: 'Oczekiwanie na uprawnienia',
    },
    ...
    settings: {
        ...
        about: 'O aplikacji',
        aboutFooter: 'Very Happy to przestrzeń Web-first dla Codex i Claude Code, dostępna w Very Happy Cloud lub we własnym wdrożeniu. Nie jest powiązana z Anthropic.',
        whatsNew: 'Co nowego',
    ...
    errors: {
        ...
        voiceConversationLimitReached: 'Osiągnąłeś maksymalną liczbę rozmów głosowych w tym miesiącu. Możliwe, że w przyszłości dodamy głosowe korzystanie na żądanie — prosimy o zgłoszenie na github.com/Mereithhh/very-happy/issues, jeśli napotkasz ten limit.',
    ...
    session: {
        ...
        completedChanges: ({ count }: { count: number }) => `${count} ${plural({ count, one: 'zmieniony plik', few: 'zmienione pliki', many: 'zmienionych plików' })}`,
        viewDiff: 'Pokaż diff',
        newChat: 'Nowy czat',
        setTitle: 'Ustaw tytuł',
        renameTitle: 'Zmień nazwę sesji',
        renamePlaceholder: 'Tytuł sesji',
    ...
    agentInput: {
        ...
        context: {
            ...
            used: ({ percent }: { percent: number }) => `${percent}% kontekstu`,
            compact: 'Skróć',
        },
        suggestion: {
            fileLabel: 'PLIK',
    ...
    permissions: {
        pendingCount: ({ count }: { count: number }) => `${count} oczekujących uprawnień`,
