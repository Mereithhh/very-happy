    ...
    changelog: {
        ...
        releases: {
            ...
            sep25n: {
                ...
                keys: '发起操作的机器从不持有账号内容密钥或对方机器的会话密钥，只有结果传回来。这是刻意选择的方案，而不是把账号密钥交给 CLI。',
                guard: '只回答五个白名单操作、只限同一账号，每台机器有速率上限，应答的 daemon 日志里留一行审计记录。在那台机器的 `~/.happy/settings.json` 里设 `remoteSessionOps: "off"` 可以关闭。',
            },
            sep25l: {
                title: '「自动化」入口不再消失',
