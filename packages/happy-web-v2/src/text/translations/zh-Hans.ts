...
export const zhHans: TranslationStructure = {
    ...
    changelog: {
        ...
        releases: {
            ...
            sep03ah: {
                title: 'daemon 会在机器空闲时自己升级',
                summary: '以前要让一台机器保持最新，得记着手动跑一次升级。现在它可以自己来，但只在「打断不了任何东西」的那个窗口里做。',
                idle: '只有机器真正空闲时才升级——没有 agent 会话、也没有在用的 web 终端，因为升级会换掉承载它们的进程。同一个版本只试一次，坏的安装不会变成无限重试。',
                pinned: '它只装 relay 推荐的那个版本，绝不是 npm 上最新的；而且新程序必须先成功启动才会接管。机器页会显示升级结果，`cliAutoUpdate` 可以关掉它。',
            },
            sep03ac: {
                title: '「刷新」按钮不会再卡住',
