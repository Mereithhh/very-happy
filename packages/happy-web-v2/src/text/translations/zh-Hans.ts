...
export const zhHans: TranslationStructure = {
    ...
    teams: {
        ...
        scheduleActive: "运行中",
        schedulePaused: "已暂停",
        scheduleCompleted: "已触发",
        nextRun: "下次触发",
        schedulePending: "消息等待指定队友接收。",
        pauseSchedule: "暂停",
        resumeSchedule: "恢复",
        cancelSchedule: "取消定时任务",
        createSchedule: "创建定时任务",
        scheduleName: "定时任务名称",
        scheduleRecipient: "接收队友",
        chooseBot: "选择队友",
        scheduleTime: "首次触发（本地时间）",
        scheduleInterval: "间隔分钟（单次留空）",
        scheduleBody: "发给队友的指令",
        invalidSchedule: "请填写有效时间、接收队友和内容；重复间隔须为 1 至 527040 的整数分钟。",
        ...
        cancelled: "已取消",
        unavailable: "当前服务器未提供团队能力，或团队已不存在。请更新服务器或返回团队列表。",
        sessions: "会话",
        teams: "团队",
        refresh: "刷新",
        retry: "重试原操作",
        intro: "让 coding agents 围绕同一个目标协作，在这里查看进展并接管。",
        name: "团队名称",
        machine: "执行机器",
        selectMachine: "选择机器",
        create: "创建团队",
        bots: "队友",
        joinHint: "关联会话只登记身份。负责人还需要安装团队 skill 和工具后才能自主委派。",
        lead: "负责人",
        openSession: "打开会话",
        starting: "等待启动",
        leadName: "负责人名称",
        ...
        unknown: "结果未知，请核对执行会话，避免重复启动。",
        assignee: "执行者",
        cleanup: "回收",
        failed: ({ reason }: { reason: string }) => `操作未完成：${reason}。请重试。`,
        counts: ({ bots, tasks }: { bots: number; tasks: number }) => `${bots} 位队友 · ${tasks} 个任务`,
    },
    board: {
        title: '任务看板',
        filterLabel: '看板',
    ...
    automations: {
        ...
        statusActive: '运行中',
        statusPaused: '已暂停',
        concurrencySkip: '忙时跳过',
        concurrencyQueue: '忙时排队',
        maxRuntime: '最长运行',
        version: 'v',
        openSession: '打开会话',
        ack: '知道了',
        ...
        resume: '恢复',
        cancelRun: '取消运行',
        cancelRunTitle: '取消这次运行？',
        cancelRunConfirm: ({ name }: { name: string }) => `停止「${name}」当前这次运行？daemon 会在下次上报时放弃它。`,
        deleteAutomation: '删除自动化',
        deleteConfirm: ({ name }: { name: string }) => `删除「${name}」及其全部运行记录？正在进行的运行会被放弃。`,
        actions: '操作',
        runQueued: ({ name }: { name: string }) => `已排队运行 ${name}`,
        acked: '已确认',
        cancelled: '已取消运行',
        paused: '已暂停',
        resumed: '已恢复',
        ...
        emptyTitle: '还没有自动化',
        emptyBody: '在任何已连接的机器上创建：very-happy auto create --name … --cron "0 9 * * *" --tz Asia/Singapore --spawn-dir ~/project --prompt "…"',
        notFound: '这条自动化已不存在',
        loadFailed: ({ code }: { code: string }) => `刷新失败：${code}。显示的是上次拿到的状态。`,
        action: '动作',
        actionSpawn: '新建会话',
        actionSend: '发送到会话',
        actionScript: '运行脚本',
        promptTruncated: '展开完整 prompt',
        promptCollapse: '收起',
        stickies: '粘性会话',
        noStickies: '没有粘性会话——每次运行都新建会话。',
        runs: '最近运行',
        noRuns: '还没有运行',
        summary: '摘要',
        error: '错误',
        ...
        sourceFire: '事件',
        sourceManual: '手动',
        reasonNeedsInput: 'Agent 在等你输入',
        reasonFailed: '运行失败',
        reasonExpired: '运行超时——daemon 停止上报或超过最长运行时间',
        reasonMachineOffline: '机器离线——没有人领取这次运行',
        reasonUnknownOutcome: 'daemon 重启，结果未知',
        reasonDaemonRestarted: '脚本运行中 daemon 重启了',
        reasonInvalidAction: 'daemon 拒绝了这个动作（未知 agent 或权限模式）',
        sidebarEntry: '自动化',
        groupScheduled: '定时',
        groupTriggered: '触发器',
        groupScheduledHint: 'cron、固定间隔、一次性',
        groupTriggeredHint: '由事件触发：very-happy auto fire、MCP automation_fire',
        fireTitle: '怎么触发',
        fireBody: '这条自动化没有时间表，由事件触发。在任何机器或会话里触发一次即可；payload 在 prompt 里以 {{payload}} / {{payload.field}} 取值，24 小时内同一 dedupe key 返回原来的运行。',
        fireCli: '在终端里',
        fireCliPayload: '带 JSON payload 与 dedupe key',
        fireMcp: '在托管会话里（MCP 工具）',
        fireRunNowHint: '上方的「立即运行」会排队一次不带 payload 的运行。',
        newAutomation: '新建自动化',
        editAutomation: '编辑',
        formTitleNew: '新建自动化',
        formTitleEdit: ({ name }: { name: string }) => `编辑 ${name}`,
        create: '创建',
        saveChanges: '保存修改',
        created: ({ name }: { name: string }) => `已创建 ${name}`,
        saved: '已保存',
        noChanges: '没有改动',
        formBasics: '基本',
        formTrigger: '触发',
        ...
        formLimits: '限制',
        fieldName: '名称',
        fieldNameHint: '小写字母、数字、- _ .；触发时用这个名字',
        fieldDescription: '描述',
        fieldMachine: '机器',
        fieldMachineHint: '由这台机器上的 daemon 执行',
        chooseMachine: '选择机器',
        triggerCron: '定时（cron）',
        triggerInterval: '固定间隔',
        triggerOnce: '一次性',
        triggerManual: '仅触发器',
        triggerManualHint: '没有时间表，由事件触发：very-happy auto fire <name> 或 MCP 工具 automation_fire。',
        fieldCron: 'cron 表达式',
        fieldCronHint: '五段：分 时 日 月 周，或 @hourly / @daily / @weekly / @monthly',
        fieldTz: '时区',
        fieldEvery: '每隔',
        fieldEveryHint: '如 15m、1h30m、1d（最小 1m）',
        fieldAt: '运行时间',
        fieldAtHint: '按当前浏览器时区',
        actionSpawnHint: '在机器上新建一个会话并发送 prompt。',
        actionSendHint: '把 prompt 发进一个已有会话（必须在这台机器上且活着）。',
        actionScriptHint: '不经 shell 直接运行命令；stdout/stderr 尾部作为摘要，exit 0 = 完成。',
        fieldAgent: 'Agent',
        fieldDirectory: '目录',
        fieldDirectoryHint: '机器上的绝对路径或 ~/…',
        fieldModel: '模型',
        fieldModelHint: '可选，留空用 agent 默认',
        fieldPermissionMode: '权限模式',
        fieldPermissionDefault: 'daemon 默认',
        fieldWorktree: '在新的 git worktree 里运行',
        fieldStickyKey: '粘性 key',
        fieldStickyKeyHint: '可选模板，如 conv-{{payload.conversationId}}；同一 key 的运行续用同一会话',
        fieldPrompt: 'Prompt',
        fieldPromptHint: '模板变量：{{payload}}、{{payload.field}}、{{run.id}}、{{automation.name}}、{{now}}',
        fieldSession: '会话 id',
        fieldCommand: '命令',
        fieldCommandHint: 'argv，按 shell 引号规则拆分、不做展开，如 python3 sync.py --since "15 min"',
        fieldCwd: '工作目录',
        fieldConcurrency: '上一次还没跑完时',
        concurrencySkipLabel: '跳过这一次',
        concurrencyQueueLabel: '排队等待',
        fieldMaxRuntime: '最长运行',
        fieldMaxRuntimeHint: '如 30m、6h；留空用默认（6h，脚本 30m）',
        errName: '只能用小写字母、数字、- _ .（1–64 位，字母或数字开头）',
        errMachine: '选一台机器',
        errCron: '需要五段 cron（或 @别名）',
        errTz: '不是有效的 IANA 时区',
        errInterval: '1m 到 366d 之间，如 15m、1h30m',
        errOnce: '选择日期和时间',
        errAgent: '选一个 agent',
        errDirectory: '目录必填',
        errPrompt: 'Prompt 必填',
        errSession: '会话 id 必填',
        errScript: '命令必填（检查引号是否配对）',
        errMaxRuntime: '1m 到 7d 之间',
        errSchema: '服务器不接受这个形状，请检查各字段',
        errNameTaken: '这个名字已被另一条自动化使用',
        errInvalidCron: '服务器拒绝了这个 cron 表达式',
        errInvalidTimezone: '服务器拒绝了这个时区',
        errMachineNotFound: '这台机器不属于当前账号',
        errStale: '已被别处修改，请重新打开再编辑',
        cancel: '取消',
        offlineMachine: '离线',
    },
    tabs: {
    ...
    session: {
        ...
        chat: {
            ...
            queueDelete: '删除排队消息',
            queueIntervene: '调整方向',
            queueDeliveryFailed: '消息未加入发送队列，草稿已保留，请重试。',
            queueMore: '排队消息选项',
    ...
    workspaceGuide: {
        ...
        groups: {
            ...
            organizeSummary: '临时笔记本、Todo 面板，以及定时或事件触发的自动化',
        ...
        capabilities: {
            ...
            automationsDescription: '从侧边栏打开自动化，在一台机器上按时或按事件启动 Agent 会话或脚本（cron + 时区、固定间隔、一次性，或由 very-happy auto fire 触发）。失败、超时或等你输入的运行会出现在任务看板的「需要我决策」区。',
            viewsTitle: '终端 ↔ 结构化文本',
    ...
    tools: {
        ...
        builtin: {
            labels: {
                ...
                team_cancel: '取消任务',
                team_handoff: '移交任务',
                team_schedule_create: '创建提醒',
                team_schedule_pause: '暂停提醒',
                team_schedule_resume: '恢复提醒',
                team_schedule_cancel: '取消提醒',
                automation_list: '列出自动化',
                automation_get: '查看自动化',
                automation_create: '创建自动化',
                automation_update: '修改自动化',
                automation_pause: '暂停自动化',
                automation_resume: '恢复自动化',
                automation_delete: '删除自动化',
                automation_run: '运行自动化',
                automation_fire: '触发自动化',
                automation_runs: '列出运行记录',
                automation_report: '汇报运行结果',
                automation_ack: '确认运行',
                sessions_list: '列出会话',
                session_read: '读取会话',
                session_send: '发消息到会话',
            ...
            fields: {
                title: '标题', text: '文本', path: '路径', mode: '模式', progress: '进度', attention: '关注',
                name: '名称', newName: '新名称', id: 'Id', machine: '机器', team: '团队', bot: 'Bot', goal: '目标',
                acceptance: '验收标准', assignee: '负责人', agent: 'Agent', model: '模型', directory: '目录',
                parentTask: '父任务', task: '任务', recipient: '接收者', body: '消息', result: '结果',
                reason: '原因', attempt: '尝试', schedule: '提醒', runAt: '运行时间', interval: '重复',
                version: '版本', description: '说明', trigger: '触发', action: '动作', prompt: 'Prompt',
                command: '命令', sticky: '续接键', concurrency: '并发', maxRuntime: '最长运行',
            ...
            trigger: {
                manual: '仅手动',
                once: ({ at }: { at: string }) => `${at} 一次`,
                onceUnknown: '一次性',
                interval: ({ every }: { every: string }) => `每 ${every}`,
                intervalUnknown: '固定间隔',
                hourly: ({ minute }: { minute: string }) => `每小时 :${minute}`,
                daily: ({ time }: { time: string }) => `每天 ${time}`,
                weekdays: ({ time }: { time: string }) => `工作日 ${time}`,
                weekly: ({ time, day }: { time: string; day: string }) => `每周第 ${day} 天 ${time}`,
                monthly: ({ time, day }: { time: string; day: string }) => `每月 ${day} 日 ${time}`,
                cron: ({ expr }: { expr: string }) => `cron ${expr}`,
                cronUnknown: 'cron',
            },
            action: {
                spawn: ({ agent, dir }: { agent: string; dir: string }) => `在 ${dir} 启动 ${agent} 会话`,
                send: ({ session }: { session: string }) => `发送到会话 ${session}`,
                script: ({ cmd }: { cmd: string }) => `运行 ${cmd}`,
            },
            status: { active: '启用', paused: '已暂停' },
            runStatus: {
                queued: '排队中', claimed: '已领取', running: '运行中', done: '完成', failed: '失败',
                skipped: '已跳过', expired: '已过期', cancelled: '已取消',
            },
            taskStatus: { queued: '排队中', running: '进行中', submitted: '已提交', done: '完成', cancelled: '已取消' },
            to: ({ target }: { target: string }) => `给 ${target}`,
            chars: ({ count }: { count: string }) => `${count} 字符`,
            lastN: ({ count }: { count: number }) => `最近 ${count} 条`,
            yes: '是',
            no: '否',
            nextRun: ({ at }: { at: string }) => `下次 ${at}`,
            teamSummary: ({ name, members }: { name: string; members: number }) => `团队 ${name} · ${members} 名成员`,
            inspectSummary: ({ name, tasks, bots, messages }: { name: string; tasks: number; bots: number; messages: number }) => `${name} · ${tasks} 个任务 · ${bots} 名成员 · ${messages} 条消息`,
            taskRef: ({ id }: { id: string }) => `任务 ${id}`,
            scheduleRef: ({ id }: { id: string }) => `提醒 ${id}`,
            automationCount: ({ count }: { count: number }) => `${count} 条自动化`,
            runCount: ({ count }: { count: number }) => `${count} 次运行`,
    ...
    changelog: {
        ...
        releases: {
            sep25i: {
                ...
                rows: '每次调用压成一行可以直接读的摘要：「委派任务 · 给 Codex · 实现自动化页面」「创建自动化 · daily-inventory · 每天 09:00 · Asia/Singapore」「进度 · 待审阅 · …」。',
                cards: '展开后按字段展示参数（目标、验收标准、触发、动作、Prompt 等）、结果摘要，以及打开对应会话、团队或任务的按钮；错误直接显示，原始输出折叠在一键之下。',
                runners: 'Claude、Codex、pi 会话表现一致；其他 MCP 服务器的工具保持原来的通用视图。',
            },
            sep25h: {
                title: 'Pi 终端会自动命名标签页',
            ...
            sep25e: {
                ...
                heartbeat: '空闲会话现在最多 30 秒刷新一次，工作中的会话 4 秒一次；开始或结束工作、离线和重新上线仍然立即显示。',
                assets: '网页的脚本和样式文件现在由 CDN 和浏览器长期缓存（每次发版文件名都会变），两次发版之间刷新页面从服务器拉取的内容少得多。',
            },
            sep25d: {
            ...
            sep10m: {
                ...
                surface: '去掉分栏处的亮色带与文件区底色差异，保留细分界、标签选中反馈和拖拽调宽。',
            },
            sep10l: {
                title: '文件预览更好用了',
            ...
            sep10a: {
                ...
                context: "Pi 上下文用量采用运行时的当前估算值和真实模型容量，未上报或压缩后未知时显示 —；其他未知模型也不再默认猜 200k 容量。更新 CLI 后，新建 pi 会话即可启用上报。",
            },
            ...
            sep09k: {
                ...
                composer: '权限、模型与主按钮在输入框内保持单排对齐，上下文用量放到输入框下方；＋菜单提供独立的快捷指令入口，已用百分比与已用/总容量 tokens 直接可见；最大思考档使用彩色进度条。底部只保留一个主按钮，在发送、排队与停止间切换，调整方向放在排队消息上。',
                queue: '排队消息贴在输入框上方，可以调整方向、删除或编辑。编辑期间暂停自动发送，保存或取消后恢复；输入为空时保留停止按钮。',
                edit: '在原消息位置直接编辑，再在新分支中发送。创建前核对准确历史，原会话保留，已执行的文件修改不会撤销。',
                actions: '工具调用之间的中间回复不再显示复制和引用；最终回复保留操作，桌面悬停或聚焦显示，触屏通过更多入口展开。',
            },
            sep09j: {
                title: '看清会话的实时进展',
            ...
            sep09: {
                ...
                messages: '引用消息时保留已有草稿。支持新接口的 Claude/Codex daemon 可从原消息之前创建分支并编辑重跑，原会话保留；旧消息可手选精确历史点位。文件修改不撤销，pi 暂未接入回退。',
                ordering: '新用户默认不分组，可直接排序；分组视图提供切换为不分组，手机说明上移/下移菜单。',
                setup: 'Todo 明确需要 provider 并提供可运行示例教程。设置和帮助页补齐更新日志、新增机器和机器更新入口。',
                symbols: '终端采用常见符号字形，让用户名和主机名中的 @ 清晰可辨，终端网格不变。',
            },
            sep08k: {
            ...
            sep08j: {
                ...
                fallback: 'Web 中继查询共用三秒期限，CLI 的查询、探测与认领共用八秒期限；超时释放连接流程，已经发出的操作不会因此重复执行。',
                diagnostics: '浏览器自动上报连接阶段和结果，断网时有界暂存、恢复后补传；服务端和中继默认保留连接断开原因、传输方式、耗时及 RPC 失败分类，指标服务也按配置启动。这些诊断不记录终端内容或凭据。',
            },
            sep08a: {
            ...
            sep08d: {
                title: '终端打开就在最后一行，并且一直跟着输出走',
                summary: '进入终端——新建、从列表点进、切回标签或刷新——有时会停在历史中间，之后新输出也不再把视图带到底。原因不在历史加载：终端区域一旦变高（软键盘收起、浏览器工具栏收起、窗口缩放），浏览器会顺手挪一下滚动位置，终端把这当成了你在往上翻。',
                follow: '原本在最后一行的终端，经过键盘开关、浏览器界面变化和窗口缩放后仍在最后一行，新输出会持续滚入视野。',
                history: '往上翻历史的行为不变：你在看历史时，新输出不会把你拉回底部，布局变化也不会。全屏程序（vim、htop）不受影响。',
            },
            sep08g: {
                title: '没被执行的排队消息现在会明说，而不是装作已经处理',
                summary: 'agent 忙的时候发出去的消息会先排队；如果 agent 随后把队列清掉了（/clear、/compact、终端模式下点停止，或者会话在轮到它之前被重启），这条消息以前会一直显示「排队中」，等下一个不相关的回合结束才悄悄变成「已处理」的样子。',
                tombstone: 'agent 现在会上报它丢弃的每一条排队消息。气泡留在对话里，下面多一行「未执行 · 原因」，可以直接复制重发。你自己从队列里移除的消息还是照旧直接消失。',
                restart: '重启会话（或另一台机器接管）时，旧进程里还没轮到的消息会被标成「未执行 · 会话重启前没来得及处理」。需要机器上的 CLI 升级；已经在跑的会话要重启后才有这个行为。',
            },
            sep08c: {
   ...
   summary: '手机上弹出/收起软键盘，之前会把终端「变小字」以塞进更多列——于是 Claude Code 启动 logo 和历史文本被重新折行，甚至打印出两遍。',
   columns: '现在键盘只会改变能塞下多少行，列数固定不变。Claude Code 的 logo 和终端历史在任何键盘状态下都保持刚打印出来的样子，不再重排。',
   seam: '终端字体的行高也始终保持紧密的 1.0，logo 方块/框线无缝拼接，不会被键盘收起后重新引入的缝隙打断。',
        },
                   sep07: {
   ...
   model: '现在选择器先显示你明确选过的模型，否则显示会话报告的当前模型，最后才是列表第一项。会话一直用的是对的模型，错的只是标签。',
     ...
     sep06a: {
            ...
            sep05a: {
                ...
                duplicates: '终端列表现在每个终端只出现一次，取真正在线的那条机器记录。打开网页就生效，不需要升级任何东西。如果还看到一条过期的机器，「设置 → 诊断 → 该机器 → 删除」可以清掉它，终端本身不受影响。',
                identity: '根因在机器上：一次读不出设置文件，和「第一次安装」在代码里长得一模一样，于是 CLI 给自己发了个新身份、注册成第二台机器。现在它读不出身份就拒绝替换并明确报错，而不是安静地重新开始；这种状态下也不会再覆盖掉你的设置。',
            },
            sep04e: {
            ...
            sep04c: {
                ...
                tables: '模型几乎总是在一行文字后面紧接着写表格，而以前这种写法会整块变成原样的竖线和横线。现在表格能正常渲染，支持列对齐，长单元格换行而不是强迫你横着拖，只有列真的放不下时才横向滚动；单元格里的换行也生效了。',
                gfm: '新支持的还有：嵌套列表、任务清单、删除线、裸链接、引用式链接和脚注。转义的竖线和缺格的数据行不会再把表格的列错开，foo_bar_baz 这类标识符也不会再有一半变成斜体。agent 正文里引用的图片以标签形式显示，不会去远程服务器加载。',
                attachments: '以前发文件会多出一条塞满机器标记的重复消息，文件本身还显示成上面一行工具调用。重复消息没有了，附件跟着你的消息一起显示，图片直接出缩略图、点开看大图。',
            },
            sep04b: {
            ...
            sep03ae: {
                ...
                yolo: 'pi 会话有 yolo 模式了：启动器里选，或传 `--permission-mode yolo`，pi 侧的策略闸就不再为常规动作发问；硬性 deny 规则（force push、生产机、集群变更）照样拦。',
            },
            sep03ab: {
