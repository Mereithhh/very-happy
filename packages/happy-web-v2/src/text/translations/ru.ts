...
export const ru: PartialTranslationStructure = {
    ...
    errors: {
        ...
        voiceConversationLimitReached: 'Вы достигли максимального количества голосовых разговоров в этом месяце. Возможно, в будущем мы добавим голосовое использование по запросу — пожалуйста, создайте заявку на github.com/Mereithhh/very-happy/issues, если вы столкнулись с этим ограничением.',
    ...
    session: {
        ...
        completedChanges: ({ count }: { count: number }) => `${count} ${plural({ count, one: 'файл изменён', few: 'файла изменено', many: 'файлов изменено' })}`,
        viewDiff: 'Показать diff',
        newChat: 'Новый чат',
        setTitle: 'Задать название',
        renameTitle: 'Переименовать сессию',
        renamePlaceholder: 'Название сессии',
    ...
    settingsAccount: {
        ...
        backupDescription: 'Сохраните резервную копию секретного ключа, чтобы восстановить аккаунт без зависимости от ретранслятора. Этот форк доверяет серверу: оператор может восстановить ключ, а вход по паролю или через связанный Google также может вернуть его. Храните ключ в менеджере паролей.',
    ...
    setPassword: {
        ...
        intro: 'Установите пароль для входа на новых устройствах. Доверенный ретранслятор получает его по TLS, хранит только salted scrypt-верификатор и после успешного входа возвращает ключ аккаунта, хранимый сервером.',
        ...
        errorReauthRequired: 'Для безопасности выйдите и войдите снова перед изменением учётных данных.',
    },
