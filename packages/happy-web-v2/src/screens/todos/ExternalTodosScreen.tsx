...
export function ExternalTodosScreen() {
    ...
    const machineGeneration = useRef(0);
    if (currentMachineRef.current !== machineId) { machineGeneration.current++; currentMachineRef.current = machineId; }
    const mounted = useRef(true);
    useEffect(() => { mounted.current = true; return () => { mounted.current = false; }; }, []);
    useEffect(() => () => { requestRef.current++; }, [machineId]);

    const load = useCallback(
        async (targetId: string, opts?: { silent?: boolean }) => {
            if (!mounted.current || currentMachineRef.current !== targetId) return;
    ...
    const onComplete = async (item: TodoItem) => {
        if (!machineId) return;
        const generation = machineGeneration.current;
        dispatchCompletion({ type: 'begin', id: item.id });
        const res = await machineTodoComplete(machineId, item.id);
        if (!mounted.current || currentMachineRef.current !== machineId || machineGeneration.current !== generation) return;
        if ('ok' in res && res.ok === false) {
    ...
    return (
        <div className="td">
            ...
            <div className="td-body">
                <p className="td-setup-body"><a href="/skills/very-happy-todo-provider/SKILL.md" target="_blank" rel="noreferrer">{t('todos.providerSkill')}</a></p>
                {state.kind === 'idle' && (
