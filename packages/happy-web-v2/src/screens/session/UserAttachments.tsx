...
function Thumbnail({ sessionId, item }: { sessionId: string; item: AttachmentItem }) {
    const ref = item.ref;
...
function AttachmentRow({ sessionId, item }: { sessionId: string; item: AttachmentItem }) {
    const previewable = item.ref !== null && isPreviewableImage(item.mimeType);
    if (previewable) return <Thumbnail sessionId={sessionId} item={item} />;
