...
describe('legacy supervisor commands', () => {
    it('keeps the original command as ordinary tool text', () => {
        expect(toolDetail({ name: 'Bash', state: 'completed', input: { command: 'vh-ledger bind T-1 s1' }, createdAt: 0, startedAt: null, completedAt: null, description: null })).toBe('vh-ledger bind T-1 s1');
    });
});
