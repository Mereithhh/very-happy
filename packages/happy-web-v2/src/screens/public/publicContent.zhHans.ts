    ...
    },
{
  ...
  integrations: {
    ...
    sections: [
      ...
      { heading: '交接到 Web 工作区的 MCP', blocks: ['基础托管 Claude 会话提供 change_title、copy_to_clipboard、open_preview 与 report_progress。托管 Codex、Gemini 与 ACP bridge 提供 change_title、copy_to_clipboard 与 open_preview。这些交接让 Agent 把本地工作变成可见 Web 状态，而不只是再打印一行终端文本。', ['仅语音助手 / 历史 assistant 变体提供：会话列表/读取/发送/派生/停止/归档、终端列表/读取/发送，以及 memory_update 和 journal_append。', '这些 Assistant 专属工具可以修改本地会话、终端、记忆与日志。应把 Assistant 及其 prompt/tool 权限视为高权限机器控制面。'], null, '这个 --scope user 注册会让该 OS 用户的每个 Claude 会话都获得 copy_to_clipboard；它不绑定 Very Happy Web 终端。在 Very Happy 终端内，它也通过本地 daemon 提供改名和预览工具。原生 Pi 运行 very-happy pi --terminal 即可加载。每个对话和终端内都有复制与预览记录，可再次复制或重新打开文件；超长内容会标明节选。工具可用性因 runner 而异，不是通用 MCP 承诺。'] },
