    ...
    },
{
  ...
  integrations: {
    ...
    sections: [
      { heading: '官方 skills：从工作台开始', blocks: ['“帮助与新手入门”提供团队安装器与官方待办 skills。网页创建团队会自动准备协作指引。终端使用时，在教程中选择 Agent、安装共享 skill，再让托管会话读取返回的路径；安装不会修改宿主的自动发现设置。', '在“待办 → 我的待办”选择“让 AI 使用我的待办”，复制完整指引与当前 server 地址；也可在已连接电脑上运行 very-happy todo skill。读取指引不会获得账号权限，请先核对 CLI 登录和 server，再读取现有待办。', '预览外部待办 provider skill'] },
