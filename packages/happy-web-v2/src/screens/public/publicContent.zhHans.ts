    ...
    },
    {
      ...
      "blocks": [
        [
          ...
          "新建团队默认“免审批”，负责人和后续分派成员不再反复请求工具许可。可在团队设置中改为“按需审批”；已有团队、现有会话和已排队操作保留当前模式。关联会话中的团队消息使用可展开卡片，消息已投递不表示 Agent 已处理。",
          "提交结果仍需验收。在团队页通过、退回修改、取消或移交；取消会关闭未完成子树。"
        ],
        "已有对话的接入和协作指引投递在后台完成，离开页面不取消接入。团队选项可配置新成员使用的 agent、模型和并发工作数；等待子任务的父任务让出名额。未合并成果会保留，不会因回收而删除。"
      ]
    },
{
  "heading": "定时发送指令",
  ...
  keyboard: {
    ...
    sections: [
      { heading: '命令面板', blocks: ['macOS 按 Command K，Windows/Linux 按 Ctrl K，可搜索操作、活跃聊天与终端。用上下方向键移动、Enter 执行、Escape 关闭。会话 #tag 过滤与侧栏搜索使用同一语法。', ['创建聊天或终端，也可以在指定目录中创建终端。', '按名称、机器、路径或会话标签跳转到聊天或终端。', '打开团队、剪贴板历史、笔记、Todo 或设置。', '触屏设备可点侧栏搜索按钮，在没有硬件键盘时打开同一命令面板。'], '命令面板不会发送终端输入。macOS 上应用级快捷键使用 Command，因此 Ctrl K、Ctrl J、Ctrl N、Ctrl R 仍保留给 readline 和真实 Agent TUI。'] },
  ...
  terminals: {
    ...
    sections: [
      ...
      { heading: '中文、方块字符与字体', blocks: [['终端使用按需加载的双宽 CJK 字体（Maple Mono CN），中文等表意文字正好占两格，不会互相重叠。首次打开会短暂显示「终端字体加载中」，之后再打开直接用缓存。', 'daemon 会为 tmux 强制 UTF-8 locale（Linux 用 C.UTF-8，macOS 用 en_US.UTF-8），即使在以 LC_ALL=C 启动的精简主机上 CJK 也按宽度 2 计算。', '行高为 1.0，Claude Code 启动 logo 和 TUI 边框能无缝拼接。']] },
  ...
  integrations: {
    ...
    sections: [
      { heading: '官方 skills：从工作台开始', blocks: ['“帮助与新手入门”提供团队安装器与官方待办 skills。网页创建团队会自动准备协作指引。终端使用时，在教程中选择 Agent、安装共享 skill，再让托管会话读取返回的路径；安装不会修改宿主的自动发现设置。', '在“待办 → 我的待办”选择“让 AI 使用我的待办”，复制完整指引与当前 server 地址；也可在已连接电脑上运行 very-happy todo skill。读取指引不会获得账号权限，请先核对 CLI 登录和 server，再读取现有待办。', '预览外部待办 provider skill', '用自动化按时或按事件启动会话'] },
      { heading: '交接到 Web 工作区的 MCP', blocks: ['基础托管 Claude 会话提供 change_title、copy_to_clipboard、open_preview 与 report_progress。托管 Codex、Gemini 与 ACP bridge 提供 change_title、copy_to_clipboard 与 open_preview。这些交接让 Agent 把本地工作变成可见 Web 状态，而不只是再打印一行终端文本。', ['仅语音助手 / 历史 assistant 变体提供：会话列表/读取/发送/派生/停止/归档、终端列表/读取/发送，以及 memory_update 和 journal_append。', '这些 Assistant 专属工具可以修改本地会话、终端、记忆与日志。应把 Assistant 及其 prompt/tool 权限视为高权限机器控制面。'], null, '这个 --scope user 注册会让该 OS 用户的每个 Claude 会话都获得 copy_to_clipboard；它不绑定 Very Happy Web 终端。在 Very Happy 终端内，它也通过本地 daemon 提供改名和预览工具。原生 Pi 执行一次 very-happy install-pi-tools 后，在 Very Happy 终端直接运行 pi 即可加载，已有 Pi 可 /reload；加 --remove 可卸载，也可用 very-happy pi --terminal 临时加载。每个对话和终端内都有复制与预览记录，可再次复制或重新打开文件；超长内容会标明节选。工具可用性因 runner 而异，不是通用 MCP 承诺。'] },
      { heading: '配置外部 Todo provider', blocks: ['“我的待办”无需配置；可选的“外部来源”读取机器本地的 provider 命令。请在实际运行该命令的 daemon 机器上，把 todoProvider 加到 ~/.happy/settings.json。由于该设置授予本地代码执行能力，Web 端不能修改。', null, '命令会收到 list、complete <id> 或 create <title>。list 输出 JSON；complete/create 以退出码表示成功。仓库在 packages/happy-cli/examples/todo-provider-jsonfile.mjs 提供零依赖示例。', '打开 Todo provider 契约与示例 ↗'] },
  ...
  automations: {
    label: '自动化', summary: '在一台机器上按时或按事件启动 Agent 会话与脚本，并在任务看板上处理需要你决策的运行。',
    sections: [
      { heading: '一个触发器、一个动作、一台机器', blocks: ['一条自动化把触发器和动作绑定到一台执行电脑。触发器：带 IANA 时区的 cron、固定间隔（最短 1 分钟）、一次性时刻，或“仅触发器”（由事件触发）。动作：在某个目录新建 Agent 会话（Claude Code、Codex、pi、Gemini 或 OpenClaw）、把 prompt 发进已有会话，或以 argv 直接运行脚本（不经 shell）。', ['每次执行都是一条运行记录：状态（queued → claimed → running → done / failed / skipped / expired / cancelled）、会话链接、摘要和是否需要你处理。', '自动化起的会话带 #automation 标签，环境里有 VH_AUTOMATION_RUN_ID / VH_AUTOMATION_NAME；脚本还会拿到 VH_AUTOMATION_PAYLOAD。', 'prompt、argv 和粘性 key 都可以用 {{payload}}、{{payload.field}}（JSON payload）、{{run.id}}、{{automation.name}} 与 {{now}}。'], '自动化是门控功能：服务器设置 VH_AUTOMATIONS_ENABLED=true 后对所有账号开放（每个账号最多 100 条自动化，进行中与已暂停合计）。未启用时侧边栏不显示入口，CLI 只提示一行，普通对话不受影响。每条自动化在你选定的一台机器上运行，不会自动跨机器路由。'] },
      { heading: '从网页、终端或会话里创建', blocks: ['在应用里打开侧边栏的“自动化”，选择“新建自动化”：名称、机器、触发器、动作和限制（上一次没跑完时怎么办、最长运行时间）。cron 字段会实时预览它的含义；机器列表标出哪些电脑在线。编辑、暂停、恢复、立即运行和删除都在同一组页面里。', null, '托管的 Claude、Codex 和 pi 会话自带 automation_* MCP 工具（automation_create、automation_update、automation_run、automation_fire、automation_runs、automation_report 等），权限与 CLI 相同。从普通终端启动的 Agent 可以读取 very-happy auto skill 打印的官方 skill；very-happy teams install --apply 会把它和团队 skill 一起安装。'] },
      { heading: '触发器：由事件触发一次运行', blocks: ['“仅触发器”的自动化没有时间表。任何能在已连接电脑上执行命令的东西都能触发它：IM 机器人、文件监听、systemd 单元或另一个 Agent。详情页给出可复制的命令。fire 是唯一的事件入口，没有入站 webhook。', null, ['payload 在 prompt 里以 {{payload}} 或 {{payload.field}} 取值。', '24 小时内同一 --dedupe-key 会返回原来的运行，而不是再跑一次。', '粘性 key 模板（如 conv-{{payload.conversationId}}）让渲染后同一 key 的事件续用同一个会话，而不是新建会话。', '网页上的“立即运行”（以及 very-happy auto run）会排队一次不带 payload 的运行；已暂停的自动化会拒绝 fire，但仍接受“立即运行”。']] },
      { heading: '需要我决策', blocks: ['任务看板顶部列出需要你处理的运行，最紧急的在前：Agent 在等你输入、失败或超时的运行、因为机器离线而没人领取的运行。每一行可以打开会话、确认、再跑一次，或取消仍未结束的运行。没有需要处理的事情时这一区不会出现。', ['运行在 Agent 上报后结束（automation_report 或 very-happy auto report --status done|failed）；没有上报时，会话本轮结束即视为完成，最后一段 assistant 文本作为摘要。', '脚本以退出为准：exit 0 为完成，其余为失败，输出尾部作为摘要。', '排队超过十分钟没有任何 daemon 领取的运行会标记为机器离线；机器恢复后照常执行。', '自动化列表显示每条的最近结果与下次运行；详情页保留最近的运行时间线、摘要和会话链接。'], '阅读完整的自动化指南 ↗'] },
    ],
  },
  security: {
