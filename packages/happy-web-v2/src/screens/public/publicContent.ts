  ...
  },
  {
    ...
    sections: [
      { heading: 'Start or import a conversation', blocks: [
        ...
        { type: 'note', text: 'Claude Code stores transcripts per working directory. A conversation that references paths which no longer exist still imports, but the agent will not find those files until you recreate them.' },
      ] },
  ...
  },
  {
    ...
    sections: [
      ...
      { heading: 'Configure an external Todo provider', blocks: [
        { type: 'p', text: 'The Todo panel reads a machine-local provider command. Add todoProvider to ~/.happy/settings.json on the machine whose daemon will run it. This setting cannot be changed from Web because it grants local code execution.' },
