  ...
  },
  {
    ...
    sections: [
      { heading: 'Command palette', blocks: [
        { type: 'p', text: 'Press Command K on macOS or Ctrl K on Windows and Linux to search actions, active chats, and terminals. Use Arrow Up and Arrow Down to move, Enter to run, and Escape to close. Chat and supported-terminal #tag filters use the same grammar as sidebar search.' },
        { type: 'list', items: ['Create a chat or terminal, including a terminal in a chosen directory.', 'Jump to a chat or terminal by name, machine, path, or session tag.', 'Open teams, clipboard history, notes, todo list, or settings.', 'On touch screens, tap the sidebar search button to open the same command palette without a hardware keyboard.'] },
  ...
  },
  {
    ...
    sections: [
      ...
      { heading: 'Session flow', blocks: [
        ...
        { type: 'list', items: ['The user authenticates the browser to the control/data server.', 'A one-time approval connects a CLI identity to the same account.', 'Structured messages remain centrally durable, while their live delivery, machine/session RPC, and terminal bytes prefer the assigned regional relay.', 'The session runner persists input before agent execution and sends output with the central server’s authoritative id/seq; central updates remain the recovery path.', 'The daemon invokes local terminal or agent processes and streams results back.'] },
      ] },
  ...
  },
  {
    ...
    sections: [
      ...
      { heading: 'MCP handoffs into the Web workspace', blocks: [
        ...
        { type: 'note', text: 'This --scope user registration gives every Claude session for that OS user copy_to_clipboard; it is not bound to a Very Happy Web terminal. Inside a Very Happy terminal, it also provides title and preview tools through the local daemon. For native Pi, run very-happy install-pi-tools once, then launch pi directly inside a Very Happy terminal (or /reload an existing Pi). Use --remove to uninstall; very-happy pi --terminal also supports temporary loading. Copy and preview history is available inside each conversation and terminal; long content is marked as an excerpt. Tool availability varies by runner and is not a universal MCP promise.' },
      ] },
      { heading: 'Configure an external Todo provider', blocks: [
        { type: 'p', text: 'My todos needs no setup. The optional External source view reads a machine-local provider command. Add todoProvider to ~/.happy/settings.json on the machine whose daemon will run it. This setting cannot be changed from Web because it grants local code execution.' },
        { type: 'code', code: '{\n  "todoProvider": {\n    "command": "/absolute/path/to/your-provider",\n    "args": ["--source", "work"],\n    "timeoutMs": 20000\n  }\n}' },
        { type: 'p', text: 'The command receives list, complete <id>, or create <title>. list prints JSON; complete/create report success by exit code. A zero-dependency example ships at packages/happy-cli/examples/todo-provider-jsonfile.mjs.' },
        { type: 'link', href: `${GITHUB_URL}/blob/main/docs/channels.md#inbound-todo-provider-external-task-lists-in-the-web-ui`, label: 'Open the Todo provider contract and example ↗' },
      ] },
      { heading: 'Configure the account webhook', blocks: [
        { type: 'p', text: 'Open Settings → Channels, enter one HTTPS endpoint, choose completion and/or permission events, then save. Saving replaces the account’s previous webhook. Delivery is best-effort and private, loopback, link-local, and redirect targets are rejected.' },
        { type: 'note', text: 'Use the webhook for notifications, not as a guaranteed job queue. The canonical contract includes the payload, session trailer, event filters, HTTPS/SSRF rules, timeout, and management API.' },
        { type: 'link', href: `${GITHUB_URL}/blob/main/docs/channels.md#outbound-account-webhook`, label: 'Open the webhook contract ↗' },
      ] },
  ...
  },
  {
    ...
    sections: [
      { heading: 'One trigger, one action, one machine', blocks: [
        ...
        { type: 'note', text: 'Automations are feature gated: the server sets VH_AUTOMATIONS_ENABLED=true and every account on it can use them (each account holds at most 100 automations, active and paused together). When the gate is off, the sidebar entry stays hidden, the CLI says so in one line and ordinary chat is unaffected. Each automation runs on the one machine you choose; there is no automatic cross-machine routing.' },
      ] },
