...
const DOC_GROUPS = [
  { label: 'Start', slugs: ['quickstart', 'keyboard', 'cli', 'cloud', 'self-hosting'] },
  ...
  return (
    <div className="pub-page docs-page">
      ...
      <main id="main-content" className="docs-layout">
        <aside id="docs-navigation" className={menuOpen ? 'is-open' : ''}><div className="eyebrow">DOCUMENTATION</div><DocNav onNavigate={() => setMenuOpen(false)} /><div className="docs-aside-note">Choose Cloud or self-hosting for your environment. <Link to="/docs/security">Privacy and security.</Link></div></aside>
...
function DocsIndex() {
  return <article className="docs-article docs-index"><div className="docs-index-hero"><div><div className="eyebrow">THE FIELD MANUAL</div><h1 tabIndex={-1}>Build from anywhere.<br />Operate with clarity.</h1><p className="docs-lead">Connect a machine, choose an agent, and keep the system healthy from Web or an installed PWA.</p></div><div className="docs-index-stat"><BookOpen size={20} aria-hidden="true" /><strong>{PUBLIC_DOCS.length}</strong><span>FOCUSED GUIDES · ONE WORKSPACE</span></div></div><section className="docs-product-proof" aria-labelledby="docs-product-proof-title"><div className="eyebrow">THE INTERFACE IN THIS GUIDE</div><h2 id="docs-product-proof-title">Learn the product you will actually use.</h2><p>The interface below uses the authenticated app's production component styles and Console tokens with sanitized fixture data.</p><ProductWorkspacePreview compact /></section><KeyboardWorkflowProof compact /><MobileContinuityProof compact />{DOC_GROUPS.map((group) => <section key={group.label}><div className="eyebrow">{group.label.toUpperCase()}</div><div className="docs-cards">{group.slugs.map((slug, index) => { const doc = PUBLIC_DOCS.find((candidate) => candidate.slug === slug)!; return <Link key={doc.slug} to={`/docs/${doc.slug}`}><span className="docs-card-number">{String(index + 1).padStart(2, '0')} / {group.label.toUpperCase()}</span><h2>{doc.label}</h2><p>{doc.summary}</p><span>Open guide <ArrowRight size={14} /></span></Link>; })}</div></section>)}</article>;
}
