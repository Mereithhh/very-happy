  ...
  return (
    <div className="pub-page docs-page">
      ...
      <main id="main-content" className="docs-layout">
        ...
        {doc ? <><DocArticle doc={doc} docs={docs} fieldGuideLabel={copy.docs.fieldGuide} adjacentLabel={copy.docs.adjacent} endLabel={copy.docs.end} /><nav className="docs-toc" aria-label={copy.docs.onThisPage}><span>{copy.docs.onThisPage}</span>{doc.sections.map((section) => <a key={section.heading} href={`#${sectionId(section.heading)}`}>{section.heading}</a>)}</nav></> : <DocsIndex docs={docs} copy={copy.docs} />}
      </main>
      <PublicFooter />
    </div>
  );
...
function DocArticle({ doc, docs, fieldGuideLabel, adjacentLabel, endLabel }: { doc: PublicDoc; docs: PublicDoc[]; fieldGuideLabel: string; adjacentLabel: string; endLabel: string }) {
