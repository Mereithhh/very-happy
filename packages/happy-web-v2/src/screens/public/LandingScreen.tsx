...
export function LandingScreen() {
  ...
  return <div className="pub-page"><PublicHeader /><main id="main-content">
    ...
    <section className="pub-flow" aria-labelledby="flow-title"><div className="eyebrow">FIRST CONNECTION</div><h2 id="flow-title">From zero to a live agent in five steps.</h2><ol><li><span>01</span><div><h3>Create an account</h3><p>Use Google or username and password on your chosen relay.</p></div></li><li><span>02</span><div><h3>Install the CLI</h3><code>{INSTALL_COMMAND}</code></div></li><li><span>03</span><div><h3>Connect the machine</h3><code>{LOGIN_COMMAND}</code></div></li><li><span>04</span><div><h3>Start the daemon</h3><code>{DAEMON_START_COMMAND}</code><p>Starts in the background.</p></div></li><li><span>05</span><div><h3>Choose an agent</h3><code>{'very-happy\nvery-happy codex'}</code><p>Or start a session from Web.</p></div></li></ol></section>
