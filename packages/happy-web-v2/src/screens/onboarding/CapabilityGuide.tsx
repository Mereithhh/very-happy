...
export function MachineWorkflowGuide() {
  ...
  const steps = zh ? [
    ...
    ['让成果留在工作区', '右上角文件入口浏览执行电脑的目录、查看代码和改动；笔记、待办、团队与自动化帮助你记录下一步并跟进成果。', '/docs/architecture', '浏览完整能力'],
  ] : [
    ...
    ['Keep the work within reach', 'Open Files in the chat header to browse the execution computer, preview code and inspect changes. Notes, todos, teams and automations help you track the next step and review results.', '/docs/architecture', 'Explore the capabilities'],
  ];
...
export function AgentSkillsGuide() {
  ...
  return <section className="cap-guide" aria-labelledby="cap-skills-title">
    ...
    <p>{zh ? '官方 skills 说明如何操作团队、待办、自动化与外部来源。按需要阅读或安装，不必全部配置后才能开始聊天。' : 'Official skills explain teams, todos, automations and external providers. Read or install what you need; none is a prerequisite for ordinary chat.'}</p>
    <div className="cap-guide__skills">
      ...
      <article><h3>{zh ? '自动化 skill' : 'Automations skill'}</h3><p>{zh ? '托管的 Claude / Codex / pi 会话自带 automation_* 工具，可以直接创建、触发和上报自动化。想让终端里直接启动的 Agent 也懂这些命令，在已连接的电脑运行下方命令查看官方 skill；very-happy teams install --apply 会把它和团队 skill 一起安装。' : 'Managed Claude / Codex / pi sessions already carry the automation_* tools to create, fire and report automations. To teach an agent started from a plain terminal, run this command on the connected computer to print the official skill; very-happy teams install --apply installs it next to the Teams skill.'}</p><Command value="very-happy auto skill"/><Link to="/docs/automations">{zh ? '自动化教程' : 'Automations guide'}<ArrowRight size={14}/></Link></article>
