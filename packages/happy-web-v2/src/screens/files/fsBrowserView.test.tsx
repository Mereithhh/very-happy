...
it('restores directory and preview within an identity, isolates sessions and directory pickers',async()=>{
 ...
 try{
  await render('one');await act(async()=>host.querySelector<HTMLButtonElement>('[aria-label="fsBrowser.showHidden"]')!.click());await act(async()=>host.querySelector<HTMLButtonElement>('[title="src"]')!.click());
  await act(async()=>host.querySelector<HTMLButtonElement>('[title="app.ts"]')!.click());
  ...
  expect(host.querySelector('[data-file]')?.getAttribute('data-file')).toBe('/repo/src/app.ts');
  await render('two');expect(host.querySelector('[data-file]')).toBeNull();expect(host.querySelector('[title="src"]')).not.toBeNull();
  ...
  expect(host.querySelector('[title="app.ts"]')).not.toBeNull();
