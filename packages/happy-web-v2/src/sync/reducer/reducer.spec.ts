...
describe('reducer', () => {
    ...
    describe('AgentState permissions', () => {
        ...
        it('should handle tool result arriving before tool call (race condition)', () => {
            ...
            expect(result1.messages).toHaveLength(0); // Should not create anything visible
            // Orphan result is stashed, not dropped (backward backfill can deliver
            // the result page before its older tool-call page).
            expect(state.pendingToolResults.has('tool-1')).toBe(true);
            ...
            expect(result2.messages).toHaveLength(1);
            if (result2.messages[0].kind === 'tool-call') {
                // Stashed orphan result is applied as soon as the tool-call shows up
                // — otherwise the tool would render "running" forever.
                expect(result2.messages[0].tool.state).toBe('completed');
                expect(result2.messages[0].tool.result).toBe('Success');
                // completedAt comes from the result message's own createdAt
                expect(result2.messages[0].tool.completedAt).toBe(1000);
            }
            expect(state.pendingToolResults.has('tool-1')).toBe(false);

            // Result arrives again (duplicate delivery) — tool is no longer
            // running, so the duplicate must not overwrite anything.
            ...
            expect(message?.tool?.completedAt).toBe(1000);
        });
    ...
    describe('pendingToolResults orphan pairing (backward backfill)', () => {
        // Backward backfill paginates newest→oldest: a page containing a
        // tool-result can be reduced before the (older) page containing its
        // tool-call. These specs pin the stash-and-flush mechanism that keeps
        // such tools from rendering "running" forever.

        function toolCallMsg(msgId: string, toolId: string, createdAt: number, name = 'Test', input: any = { test: true }, seq?: number): NormalizedMessage {
            return {
                id: msgId,
                localId: null,
                createdAt,
        ...
        function toolResultMsg(msgId: string, toolId: string, createdAt: number, content: any = 'ok', isError = false, permissions?: any, seq?: number): NormalizedMessage {
            return {
                id: msgId,
                localId: null,
                createdAt,
        ...
        it('flushes an orphan error result as error state', () => {
            const state = createReducer();
            reducer(state, [toolResultMsg('r1', 'tool-1', 2000, { error: 'boom' }, true)]);
            expect(state.pendingToolResults.has('tool-1')).toBe(true);

            const result = reducer(state, [toolCallMsg('c1', 'tool-1', 1000)]);
            expect(result.messages).toHaveLength(1);
            if (result.messages[0].kind === 'tool-call') {
                expect(result.messages[0].tool.state).toBe('error');
                expect(result.messages[0].tool.result).toEqual({ error: 'boom' });
                expect(result.messages[0].tool.completedAt).toBe(2000);
            }
            expect(state.pendingToolResults.size).toBe(0);
        });

        it('applies permissions carried by an orphan result on flush', () => {
            const state = createReducer();
            reducer(state, [toolResultMsg('r1', 'tool-1', 2000, 'done', false, {
                date: 1500,
                result: 'approved',
                mode: 'default',
                decision: 'approved'
            ...
            if (result.messages[0].kind === 'tool-call') {
                ...
                expect(result.messages[0].tool.permission?.status).toBe('approved');
                expect(result.messages[0].tool.permission?.date).toBe(1500);
                expect(result.messages[0].tool.permission?.decision).toBe('approved');
            }
        });

        it('does not let a flushed orphan overwrite a permission-denied tool', () => {
            const state = createReducer();
            // Orphan result stashed first (newest page).
            reducer(state, [toolResultMsg('r1', 'tool-1', 3000, 'late success', false)]);

            // Tool call arrives with a stored denied permission: the tool is
            // finalized as error immediately and must stay that way.
            const agentState: AgentState = {
                completedRequests: {
                    'tool-1': {
                        tool: 'Bash',
            ...
            const result = reducer(state, [toolCallMsg('c1', 'tool-1', 1000, 'Bash')], agentState);

            const toolMsg = result.messages.find(m => m.kind === 'tool-call');
            expect(toolMsg).toBeDefined();
            if (toolMsg?.kind === 'tool-call') {
                expect(toolMsg.tool.state).toBe('error');
                expect(toolMsg.tool.result).toEqual({ error: 'Not allowed' });
                expect(toolMsg.tool.permission?.status).toBe('denied');
            }
            // Stash is consumed either way — no leak.
            expect(state.pendingToolResults.size).toBe(0);
        });

        it('updates latestTodos when a TodoWrite orphan result is flushed', () => {
            const state = createReducer();
            const newTodos = [
                { id: '1', content: 'todo A', status: 'pending', priority: 'high' }
            ];
            reducer(state, [toolResultMsg('r1', 'tool-1', 2000, { newTodos }, false)]);
            expect(state.latestTodos).toBeUndefined();

            const result = reducer(state, [toolCallMsg('c1', 'tool-1', 1000, 'TodoWrite', { todos: newTodos })]);
            expect(result.messages).toHaveLength(1);
            expect(state.latestTodos?.todos).toEqual(newTodos);
            expect(state.latestTodos?.timestamp).toBe(2000);
        });

        it('pairs multiple orphans across a realistic backward-paginated backfill', () => {
            const state = createReducer();
            // Page 1 (newest): both results, DESC within page.
            reducer(state, [
                toolResultMsg('r2', 'tool-2', 4000, 'out-2', false, undefined, 40),
                toolResultMsg('r1', 'tool-1', 3000, 'out-1', false, undefined, 30)
            ]);
            expect(state.pendingToolResults.size).toBe(2);

            // Page 2 (older): both tool calls, DESC within page.
            const result = reducer(state, [
                toolCallMsg('c2', 'tool-2', 2000, 'Read', { file: 'b' }, 20),
                toolCallMsg('c1', 'tool-1', 1000, 'Read', { file: 'a' }, 10)
            ]);
            expect(state.pendingToolResults.size).toBe(0);

            const tools = result.messages.filter(m => m.kind === 'tool-call');
            expect(tools).toHaveLength(2);
            for (const t of tools) {
                if (t.kind === 'tool-call') {
                    expect(t.tool.state).toBe('completed');
                }
            }

            // And the final display order (newest-first) follows server seq.
            const bySeq = [...result.messages].sort(compareMessagesNewestFirst);
            expect(bySeq[0].seq).toBe(20);
            expect(bySeq[1].seq).toBe(10);
        });

        it('keeps a later orphan for the same tool id (last write wins in the stash)', () => {
            const state = createReducer();
            reducer(state, [toolResultMsg('r1', 'tool-1', 2000, 'first', false)]);
            reducer(state, [toolResultMsg('r2', 'tool-1', 2500, 'second', false)]);
            expect(state.pendingToolResults.size).toBe(1);

            const result = reducer(state, [toolCallMsg('c1', 'tool-1', 1000)]);
            if (result.messages[0].kind === 'tool-call') {
                expect(result.messages[0].tool.result).toBe('second');
                expect(result.messages[0].tool.completedAt).toBe(2500);
            }
        });
    });
});
