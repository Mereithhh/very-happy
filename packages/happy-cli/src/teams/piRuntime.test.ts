...
describe('official pi bridge', () => {
    it('loads without dependencies and maps only team tools through real JSON-RPC shapes', async () => {
        ...
        try {
            ...
            await factory({ on: (_event: string, handler: any) => { gate = handler; }, registerTool: (tool: any) => tools.push(tool), getAllTools: () => [] });
            expect(tools.map(tool => tool.name)).toEqual(['team_inspect']);
