...
function describeAutomationError(status: number, code: string, runStatus?: string): string {
    ...
    const known: Record<string, string> = {
        ...
        run_not_claimed: 'The run has not been claimed by its machine yet; cancel it instead of reporting.',
        status_required: 'An explicit report needs status done or failed.',
        claim_required: 'Lease, session and sticky fields are daemon-only; report just the outcome.',
        sticky_requires_session: 'A sticky key needs a session id.',
        transaction_conflict: 'The server was busy; retry the same request.',
