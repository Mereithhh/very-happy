...
export type TeamBot = { id: string; name: string; sessionId: string | null; generation: number; root: boolean; managed: boolean; assistant: 'claude' | 'codex' | 'pi-acp'; directory: string | null };
...
export type TeamState = { id: string; name: string; machineId: string; version: number; bots: TeamBot[]; tasks: TeamTask[]; messages: TeamMessage[]; operations: TeamOperation[]; createdAt: number };
