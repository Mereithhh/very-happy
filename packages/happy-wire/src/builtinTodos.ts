 ...
 */
export function createBuiltinTodoClient(credentials: { token: string }, options: BuiltinTodoClientOptions) {
    ...
    async function get(id: string): Promise<BuiltinTodoRecord | null> {
        if (!UUID.test(id)) throw new BuiltinTodoError('validation', 'Invalid Todo ID');
        const key = BUILTIN_TODO_PREFIX + id;
        const raw = await request(`/v1/kv/${encodeURIComponent(key)}`, undefined, true);
        if (raw === undefined) return null;
        const todo = object(raw) && typeof raw.value === 'string' ? decodeBuiltinTodo(raw.value, key) : null;
        if (!todo || !object(raw) || raw.key !== key || !Number.isSafeInteger(raw.version) || (raw.version as number) < 0) {
            throw new BuiltinTodoError('invalid-response', 'Invalid Todo read response');
        }
        return { ...todo, version: raw.version as number };
    }

    async function create(title: string, note = '', id?: string): Promise<BuiltinTodoRecord> {
        const cleanTitle = title.trim();
        if (id !== undefined && !UUID.test(id)) throw new BuiltinTodoError('validation', 'Invalid Todo ID');
        const payloadKey = JSON.stringify([cleanTitle, note, id]);
        let pending = pendingCreates.get(payloadKey);
        if (pending?.operation) return pending.operation;
        if (!pending) {
            const now = Date.now();
            const draft: BuiltinTodo = {
                schemaVersion: 1, id: id ?? crypto.randomUUID(), title: cleanTitle, note, status: 'open',
                order: 0, createdAt: now, updatedAt: now,
            };
            if (!parseBuiltinTodo(draft)) throw new BuiltinTodoError('validation', 'Invalid Todo title or note');
            pending = { draft, attempted: id !== undefined };
        ...
        const operation = (async (): Promise<BuiltinTodoRecord> => {
            if (entry.attempted) {
                ...
                if (existing !== undefined) {
                    ...
                    if (id && (todo.title !== cleanTitle || todo.note !== note)) throw new BuiltinTodoError('conflict', 'This Todo ID already has different content; read it before deciding');
