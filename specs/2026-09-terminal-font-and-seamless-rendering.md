...
## L6 手机上 logo 「换行/打两遍」= **列数变了**，不是 CSS，也不是字体（T-006，2026-09）

xterm 的行是绝对定位、定宽的 `.xterm-rows > div`，**物理上不可能被 CSS 折行**；你在屏幕上看到的
「换行」全部是 **tmux 已经硬折在历史里的行**。daemon 以 classic renderer（非 alt screen）跑
`claude`，头图是一次性静态历史：它本身宽度自适应（42-62 列首帧都不换行，实测），但启动后
**任何列数变化**都会让 claude 重打头图 + tmux 硬折每一行旧历史（L4）。手机上唯一会反复改列数
的是**软键盘态改字号**（已删）。铁律：**键盘只能改行数**；字号与 lineHeight 终身固定，常量
在 `termFont.ts`。机制全文见 `2026-09-terminal-render-integrity.md`「附：T-006」。
...
## B-392：终端 `@` 可辨认性（2026-09-09，629bc399 已发布）
