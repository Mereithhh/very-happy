...
### 附:T-006 手机端 Claude Code logo 换行 —— 机制与设计(2026-09)

**现象**:手机新建终端启动 Claude Code,logo/头图行出现非预期换行,且可能打印两遍。

**根因链(全部实测)**:
1. daemon 以 `CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN=1`(classic renderer)启动
   会话(`webTerminal.ts`),所以 Claude 的启动头图是**一次性打印的静态历史**,不是
   可重绘的全屏帧。
2. `tmux capture-pane`/真实 pty 实测:Claude Code 2.1.263 的头图是**宽度自适应**的
   ——只按行数(<30 行)或屏幕阅读器切「文本来宾」分支,否则打印 9 列 logo + 文本列,
   文本按列数截断(`Math.max(cols-15,20)`),42-62 列下首帧都**不换行**。
3. **任何后续列数变化**都会:claude 重打头图(静态历史里出现两份),且 tmux 把比新宽
   宽的每一条历史行**硬折行**(终端行是硬行,不可逆)。实测 62x37 → 57x42 可复现;
   **62x37 → 62x42(只变行数)不复现**。
4. 手机上 very-happy 曾随软键盘开/关改字号(12px↔11px)来买 2-3 行。字号变化 =
   格子宽度变化 = **列数变化**(Maple Mono CN advance 0.6em:430px 屏 57↔62 列;真机
   pane 当场停在 62x37 正是键盘开态)。于是每次键盘开/关都触发上面 3,logo 换行+重打。
   同套常量还带 lineHeight 1.3/1.25,而渲染器在 1.0(#161)打开——第一次键盘周期就
   重新打开 logo 缝。

**设计判定**:见 `packages/happy-web-v2/src/screens/terminal/termFont.ts` 头部。
**铁律**:软键盘只能改变行数。列宽(fontSize)与 lineHeight 每设备终身固定;
不因键盘态而变。为此删除了键盘态的「紧凑字号」分支(它换 2-3 行的收益远小于
列数变化造成的不可逆历史折行 + logo 重打),并让 lineHeight 全路径共用 1.0 常量。
截图对比(12px、lh 1.0 vs 1.3)见 T-006 工作区证据。
