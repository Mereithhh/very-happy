...
**方法论（下次终端「慢」先做这个）**：量**块数**和**密集段跨度**，不要量首尾跨度。pi 的首尾跨度
是 936ms，按「间隔 >50ms 算一次停顿」切开才看得见真相——865ms 是 node 进程在启动、**真正的绘制只有
71ms**。我第一版就是照首尾跨度报的，结论写成「源头本来就要 0.7 秒」，正好把问题的性质搞反了：
71ms 内投递 1003 条消息 ≈ 每秒 14000 条，没有 relay/浏览器吃得下，看到的「一行一行爬」是队列在排空。
