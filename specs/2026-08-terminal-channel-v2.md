...
### B''. 上线当天的设计更正：几何所有权（B-124，v0.2.48）

spec 把几何当成「照搬 v1：共享几何、latest 赢」——**这条只对 v1 成立**。v1 渲染的是
tmux 画好的绝对整屏，客户端自己的列宽纯属摆设；v2 客户端**自己换行**，于是列宽变成
了内容的一部分：ink（claude 的 TUI 渲染器）重画状态块靠「上移 N 行逐行擦」，N 按它
看到的 **pane 宽度**算，浏览器只要宽度不同就擦不干净，旧状态行残留 = 用户看到两个
状态行。实测（同一段真 claude 流，录自 100 列 pane，已进 fixture
`__fixtures__/geometry/claude-tui-100cols.bin`）：100 列回放页脚恒 1 行，80/60 列出
现 2 行。

定稿修正：**pane 尺寸是权威，客户端采纳**；且必须**在流的正确位置**采纳——改窗口
那一瞬浏览器立刻按新宽换行、pane 还没变，同样错行。带外事件表达不了这个顺序，所以
用带内私有 **OSC 6121**（`ESC ] 6121 ; cols ; rows BEL`），经 `ingest()` 下发：自动
获得 seq、进 ring、catchUp 时照常重放——对一个自己换行的客户端来说，宽度本来就该和
内容同一条流。未知 OSC 对老 web 与 daemon 自己的 headless 都是惰性的，server 仍零改动。
配套：lines 模式下 web 的 fit 只**提议**尺寸（1.5s 无确认才本地兜底），不再一动容器
就重排；「几何第三条路」（手机纯镜像不声明尺寸）因此从 enhancement 变成**天然可行**
——客户端本来就已经在按 pane 尺寸渲染了。

### B'''. 重建保真：光标与整屏（B-126，v0.2.50）

spec 的「重建后光标」判断被实测推翻。原文：「依赖 capture 批末 `refresh-client -C`
触发的重绘块自愈；list-panes 的 cursor_x/y **不在重建序列消费**（仅诊断用）」。

实测（tmux 3.7b）：**尺寸不变时 `refresh-client -C` 产生 0 条 `%output`**（尺寸变了
才有 1 条）。也就是说那个「自愈」只在尺寸恰好变化时存在。B-124 让客户端与 pane 的
几何一致之后，「尺寸不变」变成常态，于是重建后光标**再也没有被纠正过**——应用之后
每一次相对光标移动都错行、擦除落空：三段文本叠在同一行、输入框画在自己的边框上。

定稿修正，重建序列 = **历史 + 整屏 + 光标**：
- **历史**：`-S -N -E -1`，`-J` 逻辑行（客户端按自己的宽度重排，这正是宽度自适应的
  来源）；拼接前**剪掉尾部空行**（多一个换行就把整屏顶上去一行）。
- **整屏**：无 `-J` 的物理行捕获，**补齐到 pane 行数**（多则保留最后 rows 行）——
  屏幕是应用用光标寻址的东西，必须行对行还原；用 `-J` 会让它变成另一个高度。
- **尾巴**：`-p -P -C` **即使没有待处理字节也返回一个换行**，必须剥掉（同样会顶上去一行）。
- **光标**：显式 `CUP` 到 list-panes 的 `cursor_x/cursor_y`（iTerm2 十年前就是这么做的）。

硬门补充：**重建保真度集成测试**——把重建流渲染进 headless xterm，与 tmux 自己的
`capture-pane` **逐行比对 + 光标坐标全等**（含折行长行、底部方框、半行未回车）。
真站 E2E 另有一条等价断言（重新挂载后浏览器屏幕与 tmux 逐行一致）。这两条是本区域
一天返工三轮之后补上的护栏——早有它们，B-124/125/126 都能在合并前红掉。

### B''''. 传输成本是**帧数**，不是字节（B-335，v0.2.116）

Owner 实报「启动 pi / claude code 时终端一行一行地画，输入框半天出不来、或只画了一半」。
先被排除掉的两条（都留了实证，别再重查）：几何没抖（生产 daemon 日志当天所有 open 全是
`151x51`、零 resize），管线也没错（把 `%output` 灌进 headless xterm 与 `capture-pane`
逐行比对 51/51 零差异；启动中途 resize、以及「capture batch → 小快照 → 1015 条 live →
中途 `reset + full + copies` 重建」全流程复刻，仍然零差异——B-126 那类光标错位没有复发）。

真因是 `%output` 的**条数**：tmux 是「产出方每 write 一次发一条」，不做任何合并。

| 产出方（151x51，隔离 socket，2026-09-03） | 块数 | 字节 | 中位块 |
|---|---|---|---|
| `pi` 0.84.4 启动首帧 | **1029** | 11.8 KB | **9 B** |
| `seq 1 3000`（行缓冲） | 2236 | 16.9 KB | 6 B |
| `cat` 同样大的文件（块缓冲，对照） | 70 | 17.0 KB | 1024 B |

daemon 过去每块各做一次 `ingest`(base64) + ring 一条 + `encrypt` + 再 base64 + **socket 与
relaySocket 各 emit 一次**。实测一次 pi 启动：**1029 帧 / 149.7 KB**（双发 300 KB）换 11.5 KB
正文；daemon CPU 只有 11.6ms——**瓶颈从来不是加密，是消息条数**。

修法 `terminal/outputCoalescer.ts`：**合并点必须在 `ingest()` 之前**。合并后的字节与逐块拼接
逐字节相同，于是一个 seq 覆盖一帧 ≡「tmux 这次读到的就是这么大一块」，ring / replay / gap 判定
全是 seq 连续性，**线上协议与 web 侧零改动**（老 web 天然兼容）。在 emit 层合并已编号的块则要在
线上表达「一帧多个 seq」——新事件 + 能力协商，且省不掉每块一次的加密，已否决。合并规则是 Nagle 式：
**空闲后第一块零延迟直发**（实测每次按键回显都是距上次发出 400ms 的孤块，走这条路，打字手感不变），
只有突发按 16ms 窗口合并，64KB 上限，**只合并绝不丢弃或重排**（字节流有状态，转义序列跨块）。
实测同一段 400 行突发 **395 帧 → 5 帧**，4318 字节逐字节相同。

四处必须先 flush，因为缓冲里的字节**还没有 seq**——漏掉不会报错，只会在线上表现成重画/错一拍/
丢最后一屏：capture anchor 读 `seqAtAnchor` 之前（anchor 前打印的内容已在 capture 里）、
`resizeHeadless` 改尺寸之前（那些字节按旧宽度产出；这也保证几何 marker 不会排到早于它的字节前面）、
两条 transport 退出时。

**下次终端「慢」先跑 `node scripts/dev/term-burst.mjs '<命令>'`**：它在隔离 socket 里挂一个 control
client，报出块数、中位块大小和按停顿切开的密集段。之所以做成脚本而不是一段方法论——因为**照首尾跨度
下结论会把性质搞反，而且我第一版就是这么错的**：pi 的首尾是 936ms，其中 865ms 是 node 在启动，真正的
绘制只有 71ms；我据此报了「源头本来就要 0.7 秒」，实际上是 71ms 内投递 1003 条消息（≈14000 msg/s）、
relay 和浏览器都吃不下，「一行一行爬」是队列在排空。
