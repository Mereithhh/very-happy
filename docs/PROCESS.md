...
- **发布顺序**：默认 server → web → CLI（tag → npm 200 →「等 smoke 绿 → `promote` 移 `latest`」→ relay
  一分钟缓存刷新、CI 等待推荐一致 → 各机空闲时自动升级到同一个 promote 后的 `latest`）。**推荐版本与自动安装版本都不再需要手动 pin**（铁律 6 / B-348 / B-503，`CLI_AUTO_UPDATE_VERSION=latest`）；精确 pin 只用于 hold/回滚；`vh-update` 只用来
  让某一台机器立刻跟上；
...
- **「有没有上线」只能问线上，不能问会话记忆**：多会话并行下，你的 commit 常常已经被**别的会话**的发布
  顺带带上去了（2026-09-03 实例：B-320/B-322 我一直按「已合入、待发」向 Owner 汇报，实际早随
  `de77a41f` 上线，那次发布真正新增的只有 B-334）。判据不用登机器也不用翻 run 列表：
...
- **常规蓝绿切换不需要 `vh-update`**（这条曾写成「server 部署后必须 vh-update」，蓝绿落地后已作废，
  ...
  发布后 `/health` 与线上资产核对均正常。

### CI 跑在哪（2026-08-25 起：全部 GitHub 托管）

**现状**：所有 workflow 的 `runs-on` 都是托管 `ubuntu-latest` / `macos-14` / `windows-latest`，
仓库没有 self-hosted runner（`gh api repos/…/actions/runners` 为 0，无 `LINUX_RUNNER` 变量）。
仓库自 2026-08-25 公开（`ff1ed6dc` #15）后，标准托管 runner 对公开仓不计费，原先「私有仓
2000 分钟/月、macOS 10×」那套分钟账已不适用。「CI 跑在哪」永远以 `.github/workflows/*.yml` 的
`runs-on` 为准，不要信任何文档里的机器名——2026-08 这里写着 fb-us self-hosted 时，B-369 的排查
先去查了 fb-us 的出口 IP，白绕一圈。

| workflow | 触发 | 跑在哪 |
|---|---|---|
| `quality.yml` | push main / 所有 PR | 托管 Linux |
| `cli-smoke-test.yml` `smoke-linux` | push main / PR（限 `packages/happy-cli/**`）+ 每个 tag | 托管 Linux，node 20 + 24 |
| `cli-smoke-test.yml` `smoke-hosted` | **只有 tag**（或手动勾 `cross_platform`） | 托管 macOS + Windows × node 20/24 |
| `deploy-hwsg.yml` / `deploy-relays.yml` / `publish.yml` | 手动 / tag | 托管 Linux |

- `gh variable set SKIP_HOSTED_SMOKE --body 1` 仍能临时关掉 tag 的跨平台冒烟（事后
  `gh variable delete`），代价是那几个 tag 的发版信心退回 Linux 级别；公开仓下已无省钱理由，
  只在托管 macOS/Windows 队列异常时用。
- tag 会**并行**触发 `publish.yml` 和跨平台冒烟，**刻意不做 needs 硬依赖**（保留紧急发版通道，
  和 deploy 一个口径）：所以 npm 上出现新版本 ≠ 跨平台冒烟已绿，对外宣布版本前看一眼那条 run。
- 托管 runner 出口 IP 不固定，所以 vh-sg 的 22 端口对全网开放（B-369）；想收紧只能换回
  self-hosted 固定出口或改 SSM。
- **历史**（2026-08-14 → 08-25）：私有仓时期为省分钟把所有 Linux job 迁到 fb-us LXC110
  `ci-runner`（3 个实例，quality 合并成 1 个 job、持久化 pnpm store）。公开后撤回，LXC 还在但
  未注册、闲置。若将来要重新启用：注册 runner（labels `self-hosted, linux, x64`）、
  把 `runs-on` 改回，并给每个 job 保留 `timeout-minutes`（self-hosted 卡死没有天然上限）；
  workflow 里残留的 `HAPPY_HOME_DIR` 指向 job 级 temp、`setup-node` 先于 `pnpm/action-setup`
  等注释就是为它留的。fb-us 本身只有一个公网 IP 且 2026-09 有机房电力维护，不适合再当发布链路。

### CI 不可用时的本地部署应急路径

触发条件是 **GitHub Actions 整体故障或托管队列长时间不给起**（公开仓不再有配额问题）。
`scripts/ci/deploy-hwsg.sh` 要 `SSH_KEY`/`HWSG_*` 环境变量，本地没有；手工发之前必须先核验
SSH alias 指向当前 production origin **vh-sg**（`ssh vh-sg 'hostname; cat /opt/happy/release/state.env'`，
见 operations）；`vh-us` 是已退役的东京老机、`hw-sg` 是 relay，都不是 control origin，禁止用。
应急发布也不再传源码：
...
ssh vh-sg "bash -s -- ghcr.io/mereithhh/very-happy-server@$DIGEST $SHA" \
