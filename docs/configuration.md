...
仅在确认 Caddy 等入口覆盖不可信 X-Forwarded-For、控制端端口不对公网开放后，设置 `BUSINESS_AUDIT_TRUST_CLOUDFLARE=1`。此时仍要求 Fastify 解析的入口 peer 属于 Cloudflare 官方网段才采用单值合法 `CF-Connecting-IP`。否则记录实际 peer，并标明来源，不把任意请求头当作用户 IP。官方网段快照位于 `app/audit/cloudflareOrigin.ts`。
