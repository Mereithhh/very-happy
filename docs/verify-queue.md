...
|---|---|---|---|---|
| V-086 | B-208：Web 待发布 | **iOS Safari / Chrome 添加到主屏幕** | 用未安装 Very Happy 的 iPhone 分别在 Safari 与 Chrome 普通标签页打开 `https://veryhappy.dev`：安装卡片第一步应分别指向 Safari 工具栏分享、Chrome 地址栏旁分享；按“分享→添加到主屏幕→添加”完成后，从主屏幕启动应进入 standalone 模式且不再显示安装提示。另用 iPad Safari（桌面 UA）确认仍识别为 iOS 指引；系统分享面板属于 OS UI，若缺少“添加到主屏幕”，记录 iOS 与浏览器版本后排障。 | 2026-08-26 |
