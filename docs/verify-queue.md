...
| V-097 | B-234：queued message controls（待发布 Web） | **iOS/Android 排队消息全交互** | 让 agent 跑一个长 turn，软键盘保持打开连续发送 3 条：应进入 composer 上方队列且不进 transcript；编辑第 2 条、删除第 3 条；点第 2 条的干预按钮，应停止当前 turn 并优先执行修改后的内容。再排两条让其自然逐 turn 执行；横竖屏切换、320px 窄屏无裁切/横滚，按钮好点。纯文本队列刷新后保留；带图片的待发项刷新后不恢复且不得只发送残缺文字。 | 2026-08-27 |
