...
|---|---|---|---|---|
| V-104 | B-258：权限模式唯一事实源（Web + CLI 待发布） | **双设备权限模式同步 + plan 批准默认进 yolo** | 升级 mac-office daemon 并用升级后 CLI 新开 Claude 会话：在机器 A 的浏览器把模式改成 plan，机器 B 同一会话的选择器应在数秒内跟随显示 plan（不刷新页面）；空闲（agent 不在跑）时改模式也应立即生效，随后第一条消息不弹本应豁免的审批。让 Claude 进 plan 并批准计划（不在卡片上另选模式）：后续 Bash/Write 不得再弹审批，选择器显示 Yolo 模式。再用旧版 CLI（v0.2.89）开一个会话对照：Web 应回退旧行为且不报错。 | 2026-08-31 |
