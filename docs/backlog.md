...
| id | 标题 | 类型 | 来源 | 状态 | 备注 |
|---|---|---|---|---|---|
| B-124 | **TUI 状态行显示成两个**（Owner 实报）：v2 是内容流、浏览器自己换行，而 claude 的 ink 渲染器按「上移 N 行逐行擦」重画状态块，N 按**它看到的 pane 宽度**算——浏览器列宽 ≠ pane 列宽就擦不干净，旧状态行残留。已用同一段真实 claude 字节流实测证实：回放宽度=pane(100) 页脚恒 1 行；宽度 80/60 出现 **2 行**；宽度 197 仍 1 行（只有「浏览器比 pane 窄」才重复）。两条产生路径：①本地 `tmux attach`/第二台设备改了 pane 尺寸（v2 刻意不再踢客户端）②改窗口大小的**瞬间**（浏览器立刻按新宽换行，pane 要等 refresh-client 生效）。修法=几何所有权反转：pane 尺寸是权威，浏览器**采纳**它渲染（v1 靠 tmux 绝对整屏掩盖了这个问题） | bug | Owner 实报 2026-08-17 | doing | 主 agent 修复中 |
