...
| id | 标题 | 类型 | 来源 | 状态 | 备注 |
|---|---|---|---|---|---|
| B-129 | `ask_user(question, options[])` MCP 工具：claude 要人拍板时在 web 端渲染成选择卡片，而不是在终端里等文字回复（手机上现在等于没法拍板）。复用既有 `permission-request` 往返原语的形状（`apiSession.ts:47` 已有 permissionId/description/options 四件套） | feat | Owner 2026-08-17 | todo | spec `specs/2026-08-agent-tool-surface.md` D5 / P4。三个新工具里唯一需要**双向**通道的，排最后，先让 open_preview 把注入链路验稳。⚠️ 必须有强制超时降级——绝不能把 claude 挂死等人。仅 SDK/local CLI 路径（裸终端无 session） |
| B-128 | `report_progress(progress, attention)` MCP 工具 + boardAnalyzer 降级为兜底：现在看板进度是拿 `claude -p --model haiku` 起子进程**猜**当前 session 在干嘛（还配了 5min 最小间隔 + 每小时 30 次限流护栏），让 claude 自己汇报比外部 LLM 猜准得多、实时得多、且免费 | feat | Owner 2026-08-17 | todo | spec 同上 D4 / P3。**零新通道**——boardAnalyzer 产物本来就写 session metadata 的 `board` 字段（`boardAnalyzer.ts:14-17`），自报只是换数据来源。收到自报刷新「最近自报」水位（建议 15min），水位新鲜时 boardAnalyzer 跳过 haiku 调用。⚠️ 要防滥调：description 写明「阶段性节点才报」+ daemon 侧 30s 节流且超频静默丢弃不报错。只挂 http MCP（裸终端无 session metadata）——这是三条路径工具面第一次有意出现差异 |
| B-127 | `open_preview(path)` MCP 工具：claude 写完文档/报告直接把所有 web 客户端切到该文件预览，不用人去文件浏览器里翻 | feat | Owner 2026-08-17 | todo | spec 同上 D3 / P2。跨 cli+server+web 三包。关键设计：**push 只带路径不带内容**，web 收到后用既有 `machineFsRead` 自己拉——不引入任何新文件访问权限（`fs-read` 本就暴露）、绕开 1MB relay 上限、server 只转发一个字符串。展示走 singleton overlay 挂 `AppLayout.tsx:146`（`FsFileViewer` 已自包含，给 machineId+path 就能渲染，**不改 FsBrowser**）。⚠️ 发布必须严格 server→web→CLI，否则会出现「claude 报告已预览、用户端毫无反应」的静默失败。留了 `mode: 'file'\|'diff'` 参数位，diff 实现依赖 B-036 |
| B-126 | **给 claude 注入 happy 工具面指引 + tool description 改造**（B-127/128/129 的前置）：三条 claude 路径注入能力不同，其中「web 终端里手打的裸 claude」是注入真空区——而它正是 Owner 日常主力用法 | feat | Owner 2026-08-17 | todo | spec 同上 D1+D2 / P1，**可独立发布验收**。SDK/local CLI 走 `BASE_SYSTEM_PROMPT`（`systemPrompt.ts:7`）一个常量；裸终端走**已装好的** SessionStart hook——`terminal_mirror_forwarder.cjs` 现有守卫（`HAPPY_MANAGED` 未设 且 `VH_TERMINAL_ID` 已设）正好就是判据，加一个 `additionalContext` stdout 输出即可（10k 字符上限、超时不阻断）。**否掉写 `~/.claude/CLAUDE.md` 受管块**（chezmoi 源 `dot_claude/CLAUDE.md` 无 `create_` 前缀，apply 会冲掉；而 `create_settings.json` 有，所以往 settings.json 写 hook 才一直安全）与 **PATH shim**（要为 mcp/plugin/config/update/doctor 等无会话子命令做例外，漏一个就是「用户跑 `claude mcp add` 莫名报错」，且失败模式是 claude 起不来；hook 的失败模式是静默降级）。主注入面其实是 **tool description**——它随 MCP 握手下发、与谁启动 claude 无关、按需加载，三处注册（http MCP / stdio MCP / stdio bridge）必须同源常量+断言测试。⚠️ `VH_TERMINAL_ID` 有三个覆盖缺口（tmux<3.2 / attach 不补注入 / 无 tmux 回退），靠 `ptyEnv()` 里加进程级 `VH_TERMINAL_HOST` 补齐大部分，但升级前已启动的 tmux server 仍无效（发布说明写「需新开终端」） |
...
| B-036 | FilesPanel 看 diff（git diff via sessionBash + 复用 DiffView；现在只能看全文） | ux | UX走查#6 | todo | B-127 的 `open_preview` 留了 `mode:'diff'` 参数位依赖本项；本项未做时 diff 模式降级为普通预览 |
