...
| B-105 | 终端 claude 的结构化视图（可切换）：读 transcript JSONL/hooks 把 TUI 会话镜像成自渲染视图（可读性/复制/渲染稳定性）；本地交互模式的 transcript 镜像是先例 | feat | Owner 2026-08-15 | doing | spec 定稿 Final v3 + 实施回流修正；Owner /goal 投放（specs/2026-08-terminal-mirror.goal.md），本批实现中 |
