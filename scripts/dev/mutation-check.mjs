 ...
 *
 * **数出现次数不算钉住**（B-312 实踩）：`expect(src.match(/saveFoo\(/g)).toHaveLength(5)`
 * 在删掉其中一处调用后确实会红，但 mangle 改坏的是字面量里**最后一个标识符**（也就是实参），
 * `saveFoo(` 本身原封不动 —— 测试照样绿。真实回归也是这个形状：调用还在，守卫条件或实参错了。
 * 所以断言要写出「这一处**凭什么**和另外四处不同」——它前面那个 if、它传的那个变量 ——
 * 而不是被调用者的名字或一个总数。
 *
 * 用法：
