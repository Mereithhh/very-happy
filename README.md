...
- 🪝 **Webhook notifications** — per-account: point Settings → Notifications → Webhook at any HTTPS endpoint and the server POSTs `{"title","message"}` JSON when a turn completes or a session needs your input. Works great with notify gateways that forward to a group chat (e.g. `https://ntfy.example.com/api/ingest/<token>`). One webhook per account, https-only, private addresses blocked.
